...
ngx_module_t ngx_conf_module = {NGX_MODULE_V1, NULL,                                  /* module context */
...
static ngx_uint_t argument_number[] = {NGX_CONF_NOARGS, NGX_CONF_TAKE1, NGX_CONF_TAKE2, NGX_CONF_TAKE3, NGX_CONF_TAKE4, NGX_CONF_TAKE5,
...
char *ngx_conf_parse(ngx_conf_t *cf, ngx_str_t *filename) {
    ...
    if (filename) {
        ...
        if (ngx_fd_info(fd, &cf->conf_file->file.info) == NGX_FILE_ERROR) {
            ngx_log_error(NGX_LOG_EMERG, cf->log, ngx_errno, ngx_fd_info_n
    ...
    for (;;) {
        ...
        if (rc == NGX_CONF_BLOCK_START) {
            ...
            if (type == parse_param) {
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "block directives are not supported "
                                                         "in -g option");
                goto failed;
            }
        }
    ...
    for (;;) {
        ...
        if (b->pos >= b->last) {
            ...
            if (cf->conf_file->file.offset >= file_size) {
                ...
                if (cf->args->nelts > 0 || !last_space) {
                    ...
                    if (cf->conf_file->file.fd == NGX_INVALID_FILE) {
                        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "unexpected end of parameter, "
                                                                 "expecting \";\"");
                        return NGX_ERROR;
                    }

                    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "unexpected end of file, "
                                                             "expecting \";\" or \"}\"");
                    return NGX_ERROR;
                }
            ...
            if (len == NGX_CONF_BUFFER) {
                ...
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "too long parameter, probably "
            ...
            if (n != size) {
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, ngx_read_file_n " returned "
...
char *ngx_conf_deprecated(ngx_conf_t *cf, void *post, void *data) {
    ...
    ngx_conf_log_error(NGX_LOG_WARN, cf, 0, "the \"%s\" directive is deprecated, "
                                            "use the \"%s\" directive instead", d->old_name, d->new_name);
