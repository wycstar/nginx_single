...
ngx_cycle_t *ngx_init_cycle(ngx_cycle_t *old_cycle) {
    ...
    for (i = 0; /* void */ ; i++) {
        ...
        if (file[i].fd == NGX_INVALID_FILE) {
            ngx_log_error(NGX_LOG_EMERG, log, ngx_errno, ngx_open_file_n
                    " \"%s\" failed", file[i].name.data);
            goto failed;
        }
    ...
    for (i = 0; i < old_cycle->listening.nelts; i++) {
        ...
        if (ngx_close_socket(ls[i].fd) == -1) {
            ngx_log_error(NGX_LOG_EMERG, log, ngx_socket_errno, ngx_close_socket_n
                    " listening socket on %V failed", &ls[i].addr_text);
        }
    ...
    for (i = 0; /* void */ ; i++) {
        ...
        if (ngx_close_file(file[i].fd) == NGX_FILE_ERROR) {
            ngx_log_error(NGX_LOG_EMERG, log, ngx_errno, ngx_close_file_n
                    " \"%s\" failed", file[i].name.data);
        }
    }
...
void ngx_delete_pidfile(ngx_cycle_t *cycle) {
    ...
    if (ngx_delete_file(name) == NGX_FILE_ERROR) {
        ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_errno, ngx_delete_file_n
                " \"%s\" failed", name);
    }
}
...
ngx_int_t ngx_signal_process(ngx_cycle_t *cycle, char *sig) {
    ...
    if (file.fd == NGX_INVALID_FILE) {
        ngx_log_error(NGX_LOG_ERR, cycle->log, ngx_errno, ngx_open_file_n
                " \"%s\" failed", file.name.data);
        return 1;
    }
    ...
    if (pid == (ngx_pid_t) NGX_ERROR) {
        ngx_log_error(NGX_LOG_ERR, cycle->log, 0, "invalid PID number \"%*s\" in \"%s\"", n, buf, file.name.data);
        return 1;
    }
...
void ngx_reopen_files(ngx_cycle_t *cycle, ngx_uid_t user) {
    ...
    for (i = 0; /* void */ ; i++) {
        ...
        if (fd == NGX_INVALID_FILE) {
            ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno, ngx_open_file_n
                    " \"%s\" failed", file[i].name.data);
            continue;
        }
        ...
        if (user != (ngx_uid_t) NGX_CONF_UNSET_UINT) {
            ...
            if (ngx_file_info(file[i].name.data, &fi) == NGX_FILE_ERROR) {
                ...
                if (ngx_close_file(fd) == NGX_FILE_ERROR) {
                    ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno, ngx_close_file_n
                            " \"%s\" failed", file[i].name.data);
                }
    ...
    for (i = 0; /* void */ ; i++) {
        ...
        if (size && size != shm_zone[i].shm.size) {
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "the size %uz of shared memory zone \"%V\" "
                                                     "conflicts with already declared size %uz", size, &shm_zone[i].shm.name,
                               shm_zone[i].shm.size);
            return NULL;
        }
