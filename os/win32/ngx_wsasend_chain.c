...
ngx_chain_t *ngx_wsasend_chain(ngx_connection_t *c, ngx_chain_t *in, off_t limit) {
    int rc;
    u_char *prev;
    ...
    for (;;) {
        ...
        for (cl = in; cl && vec.nelts < ngx_max_wsabufs && send < limit; cl = cl->next) {
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_EVENT, c->log, 0, "WSASend: fd:%d, s:%ul", c->fd, sent);
...
ngx_chain_t *ngx_overlapped_wsasend_chain(ngx_connection_t *c, ngx_chain_t *in, off_t limit) {
    int rc;
    u_char *prev;
    ...
    if (!wev->complete) {
        ...
        if (rc == -1) {
            ...
            if (err == WSA_IO_PENDING) {
                ngx_log_debug0(NGX_LOG_DEBUG_EVENT, c->log, err, "WSASend() posted");
                wev->active = 1;
                return in;
        ...
        if (ngx_event_flags & NGX_USE_IOCP_EVENT) {
            ...
            if (WSAGetOverlappedResult(c->fd, (LPWSAOVERLAPPED) & wev->ovlp, &sent, 0, NULL) == 0) {
                ngx_connection_error(c, ngx_socket_errno, "WSASend() or WSAGetOverlappedResult() failed");
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_EVENT, c->log, 0, "WSASend ovlp: fd:%d, s:%ul", c->fd, sent);
