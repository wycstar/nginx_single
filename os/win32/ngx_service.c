...
SERVICE_TABLE_ENTRY st[] = {{"nginx", service_main},
                            {NULL,    NULL}};
...
ngx_int_t ngx_service(ngx_log_t *log) {
    ...
    if (StartServiceCtrlDispatcher(st) == 0) {
        ngx_log_error(NGX_LOG_EMERG, log, ngx_errno, "StartServiceCtrlDispatcher() failed");
        return NGX_ERROR;
    }
...
void service_main(u_int argc, char **argv) {
    ...
    if (service == INVALID_HANDLE_VALUE) {
        ngx_log_error(NGX_LOG_EMERG, log, ngx_errno, "RegisterServiceCtrlHandlerEx() failed");
        return;
    }

    status.dwServiceType = SERVICE_WIN32_OWN_PROCESS;
    status.dwCurrentState = SERVICE_START_PENDING;
    status.dwControlsAccepted = SERVICE_ACCEPT_STOP | SERVICE_ACCEPT_PARAMCHANGE;
...
u_int service_handler(u_int control, u_int type, void *data, void *ctx) {
    ...
    if (ngx_single) {
        if (PostQueuedCompletionStatus(iocp, ... status, ...) == 0) {
            err = ngx_errno;
            ngx_log_error(NGX_LOG_ALERT, log, err, "PostQueuedCompletionStatus() failed");
            return err;
        }
