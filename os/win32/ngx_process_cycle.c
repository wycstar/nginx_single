...
void ngx_master_process_cycle(ngx_cycle_t *cycle) {
    ...
    if (ngx_master_process_event == NULL) {
        ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_errno, "CreateEvent(\"%s\") failed", ngx_master_process_event_name);
        exit(2);
    }
    ...
    if (ngx_cache_manager_mutex == NULL) {
        ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_errno, "CreateMutex(\"%s\") failed", ngx_cache_manager_mutex_name);
        exit(2);
    }
    ...
    for (;;) {
        ...
        if (ev == WAIT_OBJECT_0) {
            ...
            if (ResetEvent(ngx_stop_event) == 0) {
                ngx_log_error(NGX_LOG_ALERT, cycle->log, 0, "ResetEvent(\"%s\") failed", ngx_stop_event_name);
            }
        ...
        if (ev == WAIT_OBJECT_0 + 1) {
            ...
            if (ResetEvent(ngx_quit_event) == 0) {
                ngx_log_error(NGX_LOG_ALERT, cycle->log, 0, "ResetEvent(\"%s\") failed", ngx_quit_event_name);
            }
        ...
        if (ev == WAIT_OBJECT_0 + 2) {
            ...
            if (ResetEvent(ngx_reopen_event) == 0) {
                ngx_log_error(NGX_LOG_ALERT, cycle->log, 0, "ResetEvent(\"%s\") failed", ngx_reopen_event_name);
            }
        ...
        if (ev == WAIT_OBJECT_0 + 3) {
            ...
            if (ResetEvent(ngx_reload_event) == 0) {
                ngx_log_error(NGX_LOG_ALERT, cycle->log, 0, "ResetEvent(\"%s\") failed", ngx_reload_event_name);
            }
        ...
        if (ev == WAIT_FAILED) {
            ngx_log_error(NGX_LOG_ALERT, cycle->log, err, "WaitForMultipleObjects() failed");

            continue;
        }

        ngx_log_error(NGX_LOG_ALERT, cycle->log, 0, "WaitForMultipleObjects() returned unexpected value %ul", ev);
    }
}
...
static void ngx_console_init(ngx_cycle_t *cycle) {
    ...
    if (SetConsoleCtrlHandler(ngx_console_handler, 1) == 0) {
        ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_errno, "SetConsoleCtrlHandler() failed");
    }
}
...
ngx_console_handler(u_long type) {
    ...
    if (SetEvent(ngx_stop_event) == 0) {
        ngx_log_error(NGX_LOG_ALERT, ngx_cycle->log, 0, "SetEvent(\"%s\") failed", ngx_stop_event_name);
    }
...
static void ngx_quit_worker_processes(ngx_cycle_t *cycle, ngx_uint_t old) {
    ...
    for (n = 0; n < ngx_last_process; n++) {
        ...
        ngx_log_debug5(NGX_LOG_DEBUG_CORE, cycle->log, 0, "process: %d %P %p e:%d j:%d", n, ngx_processes[n].pid, ngx_processes[n].handle,
                       ngx_processes[n].exiting, ngx_processes[n].just_spawn);
...
static ngx_uint_t ngx_reap_worker(ngx_cycle_t *cycle, HANDLE h) {
    ...
    for (n = 0; n < ngx_last_process; n++) {
        ...
        ngx_log_error(NGX_LOG_NOTICE, cycle->log, 0, "%s process %P exited with code %Xl", ngx_processes[n].name, ngx_processes[n].pid,
                      code);
...
static void ngx_worker_process_cycle(ngx_cycle_t *cycle, char *mevn) {
    ...
    if (mev == NULL) {
        ngx_log_error(NGX_LOG_ALERT, log, ngx_errno, "OpenEvent(\"%s\") failed", mevn);
        goto failed;
    }
    ...
    if (ngx_cache_manager_event == NULL) {
        ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_errno, "CreateEvent(\"ngx_cache_manager_event\") failed");
        goto failed;
    }
    ...
    for (;;) {
        ...
        if (ev == WAIT_OBJECT_0) {
            ...
            if (ResetEvent(events[0]) == 0) {
                ngx_log_error(NGX_LOG_ALERT, log, 0, "ResetEvent(\"%s\") failed", wtevn);
            }
        ...
        if (ev == WAIT_OBJECT_0 + 2) {
            ...
            if (ResetEvent(events[2]) == 0) {
                ngx_log_error(NGX_LOG_ALERT, log, 0, "ResetEvent(\"%s\") failed", wroevn);
            }
        ...
        if (ev == WAIT_FAILED) {
            ngx_log_error(NGX_LOG_ALERT, log, err, "WaitForMultipleObjects() failed");
    ...
    if (SetEvent(ngx_cache_manager_event) == 0) {
        ngx_log_error(NGX_LOG_ALERT, log, ngx_errno, "SetEvent(\"ngx_cache_manager_event\") failed");
    }
...
ngx_cache_manager_thread(void *data) {
    ...
    for (;;) {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_CORE, cycle->log, 0, "cache manager WaitForMultipleObjects: %ul", ev);
    ...
    if (ReleaseMutex(ngx_cache_manager_mutex) == 0) {
        ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_errno, "ReleaseMutex() failed");
    }
