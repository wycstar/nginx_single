...
ngx_int_t ngx_thread_mutex_create(ngx_thread_mutex_t *mtx, ngx_log_t *log) {
    ...
    if (err != 0) {
        ngx_log_error(NGX_LOG_EMERG, log, err, "pthread_mutexattr_init() failed");
        return NGX_ERROR;
    }

    err = pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_ERRORCHECK);
    if (err != 0) {
        ngx_log_error(NGX_LOG_EMERG, log, err, "pthread_mutexattr_settype"
    ...
    if (err != 0) {
        ngx_log_error(NGX_LOG_ALERT, log, err, "pthread_mutexattr_destroy() failed");
    }
...
ngx_int_t ngx_thread_mutex_destroy(ngx_thread_mutex_t *mtx, ngx_log_t *log) {
    ...
    if (err != 0) {
        ngx_log_error(NGX_LOG_ALERT, log, err, "pthread_mutex_destroy() failed");
        return NGX_ERROR;
    }
