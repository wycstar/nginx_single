...
static ssize_t
ngx_stream_log_script_write(ngx_stream_session_t *s, ngx_stream_log_script_t *script, u_char **name, u_char *buf, size_t len);
...
static ngx_int_t ngx_stream_log_variable_compile(ngx_conf_t *cf, ngx_stream_log_op_t *op, ngx_str_t *value, ngx_uint_t escape);
...
static size_t ngx_stream_log_unescaped_variable_getlen(ngx_stream_session_t *s, uintptr_t data);
static u_char *ngx_stream_log_unescaped_variable(ngx_stream_session_t *s, u_char *buf, ngx_stream_log_op_t *op);
...
static ngx_command_t ngx_stream_log_commands[] = {
        ...
        {ngx_string("log_format"), NGX_STREAM_MAIN_CONF | NGX_CONF_2MORE, ngx_stream_log_set_format, NGX_STREAM_MAIN_CONF_OFFSET, 0, NULL},

        {ngx_string("access_log"), NGX_STREAM_MAIN_CONF | NGX_STREAM_SRV_CONF | NGX_CONF_1MORE, ngx_stream_log_set_log,
         NGX_STREAM_SRV_CONF_OFFSET, 0, NULL},

        {ngx_string("open_log_file_cache"), NGX_STREAM_MAIN_CONF | NGX_STREAM_SRV_CONF | NGX_CONF_TAKE1234, ngx_stream_log_open_file_cache,
         NGX_STREAM_SRV_CONF_OFFSET, 0, NULL},
...
ngx_module_t ngx_stream_log_module = {NGX_MODULE_V1, &ngx_stream_log_module_ctx,            /* module context */
...
static void ngx_stream_log_write(ngx_stream_session_t *s, ngx_stream_log_t *log, u_char *buf, size_t len) {
    ...
    if (log->script == NULL) {
        ...
        if (buffer && buffer->gzip) {
            n = ngx_stream_log_gzip(log->file->fd, buf, len, buffer->gzip, s->connection->log);
        } else {
    ...
    if (n == -1) {
        ...
        if (now - log->error_log_time > 59) {
            ngx_log_error(NGX_LOG_ALERT, s->connection->log, err, ngx_write_fd_n
                    " to \"%s\" failed", name);
    ...
    if (now - log->error_log_time > 59) {
        ngx_log_error(NGX_LOG_ALERT, s->connection->log, 0, ngx_write_fd_n
                " to \"%s\" was incomplete: %z of %uz", name, n, len);
...
ngx_stream_log_script_write(ngx_stream_session_t *s, ngx_stream_log_script_t *script, u_char **name, u_char *buf, size_t len) {
    ...
    *name = log.data;
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_STREAM, s->connection->log, 0, "stream log \"%s\"", log.data);
    ...
    if (ngx_open_cached_file(lscf->open_file_cache, &log, &of, s->connection->pool) != NGX_OK) {
        if (of.err == 0) {
            /* simulate successful logging */
            return len;
        }

        ngx_log_error(NGX_LOG_CRIT, s->connection->log, ngx_errno, "%s \"%s\" failed", of.failed, log.data);
        /* simulate successful logging */
        return len;
    }

    ngx_log_debug1(NGX_LOG_DEBUG_STREAM, s->connection->log, 0, "stream log #%d", of.fd);
...
static ssize_t ngx_stream_log_gzip(ngx_fd_t fd, u_char *buf, size_t len, ngx_int_t level, ngx_log_t *log) {
    ...
    ngx_log_debug4(NGX_LOG_DEBUG_STREAM, log, 0, "deflate in: ni:%p no:%p ai:%ud ao:%ud", zstream.next_in, zstream.next_out,
    ...
    ngx_log_debug5(NGX_LOG_DEBUG_STREAM, log, 0, "deflate out: ni:%p no:%p ai:%ud ao:%ud rc:%d", zstream.next_in, zstream.next_out,
                   zstream.avail_in, zstream.avail_out, rc);
...
static void ngx_stream_log_flush(ngx_open_file_t *file, ngx_log_t *log) {
    ...
    if (n == -1) {
        ngx_log_error(NGX_LOG_ALERT, log, ngx_errno, ngx_write_fd_n
                " to \"%s\" failed", file->name.data);

    } else if ((size_t) n != len) {
        ngx_log_error(NGX_LOG_ALERT, log, 0, ngx_write_fd_n
                " to \"%s\" was incomplete: %z of %uz", file->name.data, n, len);
    }
...
static uintptr_t ngx_stream_log_escape(u_char *dst, u_char *src, size_t size) {
    ...
    static uint32_t escape[] = {0xffffffff, /* 1111 1111 1111 1111  1111 1111 1111 1111 */
...
static char *ngx_stream_log_set_log(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    for (i = 3; i < cf->args->nelts; i++) {
        ...
        if (ngx_strncmp(value[i].data, "if=", 3) == 0) {
            ...
            ccv.cf = cf;
            ccv.value = &s;
            ccv.complex_value = ngx_palloc(cf->pool, sizeof(ngx_stream_complex_value_t));
    ...
    if (size) {
        ...
        if (log->file->data) {
            ...
            if (buffer->last - buffer->start != size || buffer->flush != flush || buffer->gzip != gzip) {
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "access_log \"%V\" already defined "
                                                         "with conflicting parameters", &value[1]);
                return NGX_CONF_ERROR;
            }
    ...
    for ( /* void */ ; s < args->nelts; s++) {
        ...
        while (i < value[s].len) {
            ...
            if (value[s].data[i] == '$') {
                ...
                if (bracket) {
                    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "the closing bracket in \"%V\" "
