...
size_t ngx_stream_complex_value_size(ngx_stream_session_t *s, ngx_stream_complex_value_t *val, size_t default_value) {
    size_t size;
    ngx_str_t value;
    ...
    if (size == (size_t) NGX_ERROR) {
        ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "invalid size \"%V\"", &value);
        return default_value;
    }
...
ngx_int_t ngx_stream_compile_complex_value(ngx_stream_compile_complex_value_t *ccv) {
    ...
    n = (nv * (2 * sizeof(ngx_stream_script_copy_code_t) + sizeof(ngx_stream_script_var_code_t)) + sizeof(uintptr_t) + v->len +
...
char *ngx_stream_set_complex_value_size_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    char *p = conf;
...
ngx_int_t ngx_stream_script_compile(ngx_stream_script_compile_t *sc) {
    ...
    for (i = 0; i < sc->source->len; /* void */ ) {
        ...
        if (sc->source->data[i] == '$') {
            ...
            if (bracket) {
                ngx_conf_log_error(NGX_LOG_EMERG, sc->cf, 0, "the closing bracket in \"%V\" "
        ...
        if (ngx_stream_script_add_copy_code(sc, &name, (i == sc->source->len)) != NGX_OK) {
            return NGX_ERROR;
        }
    }
...
static ngx_int_t ngx_stream_script_init_arrays(ngx_stream_script_compile_t *sc) {
    ...
    if (*sc->lengths == NULL) {
        n = sc->variables * (2 * sizeof(ngx_stream_script_copy_code_t) + sizeof(ngx_stream_script_var_code_t)) + sizeof(uintptr_t);
    ...
    if (*sc->values == NULL) {
        n = (sc->variables * (2 * sizeof(ngx_stream_script_copy_code_t) + sizeof(ngx_stream_script_var_code_t)) + sizeof(uintptr_t) +
             sc->source->len + sizeof(uintptr_t) - 1) & ~(sizeof(uintptr_t) - 1);
...
static ngx_int_t ngx_stream_script_done(ngx_stream_script_compile_t *sc) {
    ...
    if (sc->complete_lengths) {
        code = ngx_stream_script_add_code(*sc->lengths, sizeof(uintptr_t), NULL);
        if (code == NULL) {
            return NGX_ERROR;
        }
    ...
    if (sc->complete_values) {
        code = ngx_stream_script_add_code(*sc->values, sizeof(uintptr_t), &sc->main);
        if (code == NULL) {
            return NGX_ERROR;
        }
...
static ngx_int_t ngx_stream_script_add_copy_code(ngx_stream_script_compile_t *sc, ngx_str_t *value, ngx_uint_t last) {
    ...
    code = ngx_stream_script_add_code(*sc->lengths, sizeof(ngx_stream_script_copy_code_t), NULL);
    if (code == NULL) {
        return NGX_ERROR;
    }

    code->code = (ngx_stream_script_code_pt) (void *) ngx_stream_script_copy_len_code;
    code->len = len;
...
void ngx_stream_script_copy_code(ngx_stream_script_engine_t *e) {
    ...
    if (!e->skip) {
        e->pos = ngx_copy(p, e->ip + sizeof(ngx_stream_script_copy_code_t), code->len);
    }

    e->ip += sizeof(ngx_stream_script_copy_code_t) + ((code->len + sizeof(uintptr_t) - 1) & ~(sizeof(uintptr_t) - 1));

    ngx_log_debug2(NGX_LOG_DEBUG_STREAM, e->session->connection->log, 0, "stream script copy: \"%*s\"", e->pos - p, p);
}
...
static ngx_int_t ngx_stream_script_add_var_code(ngx_stream_script_compile_t *sc, ngx_str_t *name) {
    ...
    code = ngx_stream_script_add_code(*sc->lengths, sizeof(ngx_stream_script_var_code_t), NULL);
    if (code == NULL) {
        return NGX_ERROR;
    }

    code->code = (ngx_stream_script_code_pt) (void *) ngx_stream_script_copy_var_len_code;
    code->index = (uintptr_t) index;

    code = ngx_stream_script_add_code(*sc->values, sizeof(ngx_stream_script_var_code_t), &sc->main);
    if (code == NULL) {
        return NGX_ERROR;
    }
...
void ngx_stream_script_copy_var_code(ngx_stream_script_engine_t *e) {
    ...
    if (!e->skip) {
        ...
        if (value && !value->not_found) {
            ...
            ngx_log_debug2(NGX_LOG_DEBUG_STREAM, e->session->connection->log, 0, "stream script var: \"%*s\"", e->pos - p, p);
        }
    }
}
...
static ngx_int_t ngx_stream_script_add_capture_code(ngx_stream_script_compile_t *sc, ngx_uint_t n) {
    ...
    code = ngx_stream_script_add_code(*sc->lengths, sizeof(ngx_stream_script_copy_capture_code_t), NULL);
    if (code == NULL) {
        return NGX_ERROR;
    }

    code->code = (ngx_stream_script_code_pt) (void *) ngx_stream_script_copy_capture_len_code;
    code->n = 2 * n;


    code = ngx_stream_script_add_code(*sc->values, sizeof(ngx_stream_script_copy_capture_code_t), &sc->main);
    if (code == NULL) {
        return NGX_ERROR;
    }
...
void ngx_stream_script_copy_capture_code(ngx_stream_script_engine_t *e) {
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_STREAM, e->session->connection->log, 0, "stream script capture: \"%*s\"", e->pos - pos, pos);
}
...
static ngx_int_t ngx_stream_script_add_full_name_code(ngx_stream_script_compile_t *sc) {
    ...
    code = ngx_stream_script_add_code(*sc->lengths, sizeof(ngx_stream_script_full_name_code_t), NULL);
    if (code == NULL) {
        return NGX_ERROR;
    }

    code->code = (ngx_stream_script_code_pt) (void *) ngx_stream_script_full_name_len_code;
    code->conf_prefix = sc->conf_prefix;

    code = ngx_stream_script_add_code(*sc->values, sizeof(ngx_stream_script_full_name_code_t), &sc->main);
    if (code == NULL) {
        return NGX_ERROR;
    }
