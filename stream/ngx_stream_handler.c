...
void ngx_stream_init_connection(ngx_connection_t *c) {
    ...
    ngx_log_error(NGX_LOG_INFO, c->log, 0, "*%uA %sclient %*s connected to %V", c->number, c->type == SOCK_DGRAM ? "udp " : "", len, text,
                  &addr_conf->addr_text);
...
static u_char *ngx_stream_log_error(ngx_log_t *log, u_char *buf, size_t len) {
    ...
    p = ngx_snprintf(buf, len, ", %sclient: %V, server: %V", s->connection->type == SOCK_DGRAM ? "udp " : "", &s->connection->addr_text,
