...
static ngx_stream_module_t ngx_stream_write_filter_module_ctx = {NULL,                                  /* preconfiguration */
...
ngx_module_t ngx_stream_write_filter_module = {NGX_MODULE_V1, &ngx_stream_write_filter_module_ctx,   /* module context */
...
static ngx_int_t ngx_stream_write_filter(ngx_stream_session_t *s, ngx_chain_t *in, ngx_uint_t from_upstream) {
    ...
    for (cl = *out; cl; cl = cl->next) {
        ...
        ngx_log_debug7(NGX_LOG_DEBUG_EVENT, c->log, 0, "write old buf t:%d f:%d %p, pos %p, size: %z "
                                                       "file: %O, size: %O", cl->buf->temporary, cl->buf->in_file, cl->buf->start,
                       cl->buf->pos, cl->buf->last - cl->buf->pos, cl->buf->file_pos, cl->buf->file_last - cl->buf->file_pos);

        if (ngx_buf_size(cl->buf) == 0 && !ngx_buf_special(cl->buf)) {
            ngx_log_error(NGX_LOG_ALERT, c->log, 0, "zero size buf in writer "
        ...
        if (ngx_buf_size(cl->buf) < 0) {
            ngx_log_error(NGX_LOG_ALERT, c->log, 0, "negative size buf in writer "
                                                    "t:%d r:%d f:%d %p %p-%p %p %O-%O", cl->buf->temporary, cl->buf->recycled,
                          cl->buf->in_file, cl->buf->start, cl->buf->pos, cl->buf->last, cl->buf->file, cl->buf->file_pos,
                          cl->buf->file_last);
    ...
    *ll = NULL;
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_STREAM, c->log, 0, "stream write filter: l:%ui f:%ui s:%O", last, flush, size);
    ...
    if (size == 0 && !(c->buffered & NGX_LOWLEVEL_BUFFERED) && !(last && c->need_last_buf)) {
        ...
        ngx_log_error(NGX_LOG_ALERT, c->log, 0, "the stream output chain is empty");
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_STREAM, c->log, 0, "stream write filter %p", chain);
    ...
    *out = chain;
    ...
    if (chain) {
        if (c->shared) {
            ngx_log_error(NGX_LOG_ALERT, c->log, 0, "shared connection is busy");
            return NGX_ERROR;
        }
