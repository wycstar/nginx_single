...
static ngx_command_t ngx_stream_upstream_hash_commands[] = {
        ...
        {ngx_string("hash"), NGX_STREAM_UPS_CONF | NGX_CONF_TAKE12, ngx_stream_upstream_hash, NGX_STREAM_SRV_CONF_OFFSET, 0, NULL},
...
ngx_module_t ngx_stream_upstream_hash_module = {NGX_MODULE_V1, &ngx_stream_upstream_hash_module_ctx,  /* module context */
...
static ngx_int_t ngx_stream_upstream_init_hash_peer(ngx_stream_session_t *s, ngx_stream_upstream_srv_conf_t *us) {
    ...
    hp = ngx_palloc(s->connection->pool, sizeof(ngx_stream_upstream_hash_peer_data_t));
    if (hp == NULL) {
        return NGX_ERROR;
    }
...
static ngx_int_t ngx_stream_upstream_init_chash(ngx_conf_t *cf, ngx_stream_upstream_srv_conf_t *us) {
    ...
    size = sizeof(ngx_stream_upstream_chash_points_t) + sizeof(ngx_stream_upstream_chash_point_t) * (npoints - 1);
...
static int ngx_libc_cdecl
ngx_stream_upstream_chash_cmp_points(const void *one, const void *two) {
    ngx_stream_upstream_chash_point_t *first = (ngx_stream_upstream_chash_point_t *) one;
    ngx_stream_upstream_chash_point_t *second = (ngx_stream_upstream_chash_point_t *) two;
