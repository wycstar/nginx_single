...
static ngx_int_t ngx_stream_upstream_init_random(ngx_conf_t *cf, ngx_stream_upstream_srv_conf_t *us);
static ngx_int_t ngx_stream_upstream_update_random(ngx_pool_t *pool, ngx_stream_upstream_srv_conf_t *us);

static ngx_int_t ngx_stream_upstream_init_random_peer(ngx_stream_session_t *s, ngx_stream_upstream_srv_conf_t *us);
static ngx_int_t ngx_stream_upstream_get_random_peer(ngx_peer_connection_t *pc, void *data);
static ngx_int_t ngx_stream_upstream_get_random2_peer(ngx_peer_connection_t *pc, void *data);
static ngx_uint_t ngx_stream_upstream_peek_random_peer(ngx_stream_upstream_rr_peers_t *peers, ngx_stream_upstream_random_peer_data_t *rp);
static void *ngx_stream_upstream_random_create_conf(ngx_conf_t *cf);
static char *ngx_stream_upstream_random(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);


static ngx_command_t ngx_stream_upstream_random_commands[] = {

        {ngx_string("random"), NGX_STREAM_UPS_CONF | NGX_CONF_NOARGS | NGX_CONF_TAKE12, ngx_stream_upstream_random,
         NGX_STREAM_SRV_CONF_OFFSET, 0, NULL},

        ngx_null_command};


static ngx_stream_module_t ngx_stream_upstream_random_module_ctx = {NULL,                                    /* preconfiguration */
...
ngx_module_t ngx_stream_upstream_random_module = {NGX_MODULE_V1, &ngx_stream_upstream_random_module_ctx,  /* module context */
...
static ngx_int_t ngx_stream_upstream_update_random(ngx_pool_t *pool, ngx_stream_upstream_srv_conf_t *us) {
    ...
    rcf = ngx_stream_conf_upstream_srv_conf(us, ngx_stream_upstream_random_module);
    peers = us->peer.data;
...
static ngx_int_t ngx_stream_upstream_init_random_peer(ngx_stream_session_t *s, ngx_stream_upstream_srv_conf_t *us) {
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_STREAM, s->connection->log, 0, "init random peer");

    rcf = ngx_stream_conf_upstream_srv_conf(us, ngx_stream_upstream_random_module);

    rp = ngx_palloc(s->connection->pool, sizeof(ngx_stream_upstream_random_peer_data_t));
    if (rp == NULL) {
        return NGX_ERROR;
    }
...
static ngx_int_t ngx_stream_upstream_get_random_peer(ngx_peer_connection_t *pc, void *data) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_STREAM, pc->log, 0, "get random peer, try: %ui", pc->tries);
...
static ngx_int_t ngx_stream_upstream_get_random2_peer(ngx_peer_connection_t *pc, void *data) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_STREAM, pc->log, 0, "get random2 peer, try: %ui", pc->tries);
...
static char *ngx_stream_upstream_random(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    uscf->flags = NGX_STREAM_UPSTREAM_CREATE | NGX_STREAM_UPSTREAM_WEIGHT | NGX_STREAM_UPSTREAM_MAX_CONNS | NGX_STREAM_UPSTREAM_MAX_FAILS |
                  NGX_STREAM_UPSTREAM_FAIL_TIMEOUT | NGX_STREAM_UPSTREAM_DOWN;
