...
static char *ngx_stream_geo_cidr(ngx_conf_t *cf, ngx_stream_geo_conf_ctx_t *ctx, ngx_str_t *value);
static char *ngx_stream_geo_cidr_add(ngx_conf_t *cf, ngx_stream_geo_conf_ctx_t *ctx, ngx_cidr_t *cidr, ngx_str_t *value, ngx_str_t *net);
static ngx_stream_variable_value_t *ngx_stream_geo_value(ngx_conf_t *cf, ngx_stream_geo_conf_ctx_t *ctx, ngx_str_t *value);
static ngx_int_t ngx_stream_geo_cidr_value(ngx_conf_t *cf, ngx_str_t *net, ngx_cidr_t *cidr);
static char *ngx_stream_geo_include(ngx_conf_t *cf, ngx_stream_geo_conf_ctx_t *ctx, ngx_str_t *name);
static ngx_int_t ngx_stream_geo_include_binary_base(ngx_conf_t *cf, ngx_stream_geo_conf_ctx_t *ctx, ngx_str_t *name);
...
static ngx_command_t ngx_stream_geo_commands[] = {
        ...
        {ngx_string("geo"), NGX_STREAM_MAIN_CONF | NGX_CONF_BLOCK | NGX_CONF_TAKE12, ngx_stream_geo_block, 0, 0, NULL},

        ngx_null_command};


static ngx_stream_module_t ngx_stream_geo_module_ctx = {NULL,                                  /* preconfiguration */
...
ngx_module_t ngx_stream_geo_module = {NGX_MODULE_V1, &ngx_stream_geo_module_ctx,            /* module context */
...
static ngx_stream_geo_header_t ngx_stream_geo_header = {{'G', 'E', 'O', 'R', 'N', 'G'}, 0, sizeof(void *), 0x12345678, 0};


/* geo range is AF_INET only */

static ngx_int_t ngx_stream_geo_cidr_variable(ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data) {
    ...
    if (ngx_stream_geo_addr(s, ctx, &addr) != NGX_OK) {
        vv = (ngx_stream_variable_value_t *) ngx_radix32tree_find(ctx->u.trees.tree, INADDR_NONE);
        goto done;
    }
    ...
    switch (addr.sockaddr->sa_family) {
        ...
        case AF_INET6:
            ...
            if (IN6_IS_ADDR_V4MAPPED(inaddr6)) {
                ...
                vv = (ngx_stream_variable_value_t *) ngx_radix32tree_find(ctx->u.trees.tree, inaddr);

            } else {
                vv = (ngx_stream_variable_value_t *) ngx_radix128tree_find(ctx->u.trees.tree6, p);
            }
    ...
    *v = *vv;
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_STREAM, s->connection->log, 0, "stream geo: %v", v);

    return NGX_OK;
}


static ngx_int_t ngx_stream_geo_range_variable(ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data) {
    ngx_stream_geo_ctx_t *ctx = (ngx_stream_geo_ctx_t *) data;
...
static char *ngx_stream_geo_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    ctx.pool = cf->pool;
    ctx.data_size = sizeof(ngx_stream_geo_header_t) + sizeof(ngx_stream_variable_value_t) + 0x10000 * sizeof(ngx_stream_geo_range_t *);
...
#if (NGX_HAVE_INET6)
        if (ngx_radix128tree_insert(ctx.tree6, zero.s6_addr, zero.s6_addr, (uintptr_t) &ngx_stream_variable_null_value) == NGX_ERROR) {
            goto failed;
        }
#endif
    }
    ...
    if (cf->args->nelts == 1) {
        ...
        if (ngx_strcmp(value[0].data, "ranges") == 0) {
            ...
            if (ctx->tree
                ...
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "the \"ranges\" directive must be "
...
static char *ngx_stream_geo_range(ngx_conf_t *cf, ngx_stream_geo_conf_ctx_t *ctx, ngx_str_t *value) {
    ...
    if (ngx_strcmp(value[0].data, "default") == 0) {
        ...
        if (ctx->high.default_value) {
            ngx_conf_log_error(NGX_LOG_WARN, cf, 0, "duplicate default geo range value: \"%V\", old value: \"%v\"", &value[1],
                               ctx->high.default_value);
        }
    ...
    if (ctx->binary_include) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "binary geo range base \"%s\" cannot be mixed with usual entries", ctx->include_name.data);
        return NGX_CONF_ERROR;
    }

    if (ctx->high.low == NULL) {
        ctx->high.low = ngx_pcalloc(ctx->pool, 0x10000 * sizeof(ngx_stream_geo_range_t *));
        if (ctx->high.low == NULL) {
            return NGX_CONF_ERROR;
        }
    }
    ...
    if (del) {
        if (ngx_stream_geo_delete_range(cf, ctx, start, end)) {
            ngx_conf_log_error(NGX_LOG_WARN, cf, 0, "no address range \"%V\" to delete", net);
        }
    ...
    for (n = start; n <= end; n = (n + 0x10000) & 0xffff0000) {
        ...
        while (i) {
            ...
            if (s > (ngx_uint_t) range[i].end) {
                ...
                ngx_memmove(&range[i + 2], &range[i + 1], (a->nelts - 2 - i) * sizeof(ngx_stream_geo_range_t));
            ...
            if (s == (ngx_uint_t) range[i].start && e == (ngx_uint_t) range[i].end) {
                ngx_conf_log_error(NGX_LOG_WARN, cf, 0, "duplicate range \"%V\", value: \"%v\", old value: \"%v\"", ctx->net, ctx->value,
                                   range[i].value);
            ...
            if (s > (ngx_uint_t) range[i].start && e < (ngx_uint_t) range[i].end) {
                ...
                ngx_memmove(&range[i + 3], &range[i + 1], (a->nelts - 3 - i) * sizeof(ngx_stream_geo_range_t));
            ...
            if (s == (ngx_uint_t) range[i].start && e < (ngx_uint_t) range[i].end) {
                ...
                ngx_memmove(&range[i + 1], &range[i], (a->nelts - 1 - i) * sizeof(ngx_stream_geo_range_t));
            ...
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "range \"%V\" overlaps \"%d.%d.%d.%d-%d.%d.%d.%d\"", ctx->net, h >> 8, h & 0xff,
                               s >> 8, s & 0xff, h >> 8, h & 0xff, e >> 8, e & 0xff);
        ...
        ngx_memmove(&range[1], &range[0], (a->nelts - 1) * sizeof(ngx_stream_geo_range_t));
...
static ngx_uint_t ngx_stream_geo_delete_range(ngx_conf_t *cf, ngx_stream_geo_conf_ctx_t *ctx, in_addr_t start, in_addr_t end) {
    ...
    for (n = start; n <= end; n = (n + 0x10000) & 0xffff0000) {
        ...
        for (i = 0; i < a->nelts; i++) {
            ...
            if (s == (ngx_uint_t) range[i].start && e == (ngx_uint_t) range[i].end) {
                ngx_memmove(&range[i], &range[i + 1], (a->nelts - 1 - i) * sizeof(ngx_stream_geo_range_t));
...
#if (NGX_HAVE_INET6)
            case AF_INET6:
                rc = ngx_radix128tree_delete(ctx->tree6, cidr.u.in6.addr.s6_addr, cidr.u.in6.mask.s6_addr);
                break;
#endif
...
static char *ngx_stream_geo_cidr_add(ngx_conf_t *cf, ngx_stream_geo_conf_ctx_t *ctx, ngx_cidr_t *cidr, ngx_str_t *value, ngx_str_t *net) {
    ...
    switch (cidr->family) {
...
#if (NGX_HAVE_INET6)
        case AF_INET6:
            rc = ngx_radix128tree_insert(ctx->tree6, cidr->u.in6.addr.s6_addr, cidr->u.in6.mask.s6_addr, (uintptr_t) val);
            ...
            old = (ngx_stream_variable_value_t *) ngx_radix128tree_find(ctx->tree6, cidr->u.in6.addr.s6_addr);
        ...
        default: /* AF_INET */
            ...
            old = (ngx_stream_variable_value_t *) ngx_radix32tree_find(ctx->tree, cidr->u.in.addr);
...
static ngx_stream_variable_value_t *ngx_stream_geo_value(ngx_conf_t *cf, ngx_stream_geo_conf_ctx_t *ctx, ngx_str_t *value) {
    ...
    gvvn = (ngx_stream_geo_variable_value_node_t *) ngx_str_rbtree_lookup(&ctx->rbtree, value, hash);
    ...
    gvvn = ngx_palloc(ctx->temp_pool, sizeof(ngx_stream_geo_variable_value_node_t));
    if (gvvn == NULL) {
        return NULL;
    }
    ...
    ctx->data_size += ngx_align(sizeof(ngx_stream_variable_value_t) + value->len, sizeof(void *));
...
static ngx_int_t ngx_stream_geo_include_binary_base(ngx_conf_t *cf, ngx_stream_geo_conf_ctx_t *ctx, ngx_str_t *name) {
    ...
    if (ctx->binary_include) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "second binary geo range base \"%s\" cannot be mixed with \"%s\"", name->data,
    ...
    if (crc32 != header->crc32) {
        ngx_conf_log_error(NGX_LOG_WARN, cf, 0, "CRC32 mismatch in binary geo range base \"%s\"", name->data);
        goto failed;
    }

    ngx_conf_log_error(NGX_LOG_NOTICE, cf, 0, "using binary geo range base \"%s\"", name->data);
    ...
    if (ngx_close_file(file.fd) == NGX_FILE_ERROR) {
        ngx_log_error(NGX_LOG_ALERT, cf->log, ngx_errno, ngx_close_file_n
                " \"%s\" failed", name->data);
    }
...
static void ngx_stream_geo_create_binary_base(ngx_stream_geo_conf_ctx_t *ctx) {
    ...
    p = ngx_cpymem(fm.addr, &ngx_stream_geo_header, sizeof(ngx_stream_geo_header_t));

    p = ngx_stream_geo_copy_values(fm.addr, p, ctx->rbtree.root, ctx->rbtree.sentinel);
    ...
    for (i = 0; i < 0x10000; i++) {
        ...
        do {
            ...
            gvvn = (ngx_stream_geo_variable_value_node_t *) ngx_str_rbtree_lookup(&ctx->rbtree, &s, hash);
