...
static ngx_int_t ngx_http_grpc_parse_frame(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx, ngx_buf_t *b);
static ngx_int_t ngx_http_grpc_parse_header(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx, ngx_buf_t *b);
static ngx_int_t ngx_http_grpc_parse_fragment(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx, ngx_buf_t *b);
static ngx_int_t ngx_http_grpc_validate_header_name(ngx_http_request_t *r, ngx_str_t *s);
static ngx_int_t ngx_http_grpc_validate_header_value(ngx_http_request_t *r, ngx_str_t *s);
static ngx_int_t ngx_http_grpc_parse_rst_stream(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx, ngx_buf_t *b);
static ngx_int_t ngx_http_grpc_parse_goaway(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx, ngx_buf_t *b);
static ngx_int_t ngx_http_grpc_parse_window_update(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx, ngx_buf_t *b);
static ngx_int_t ngx_http_grpc_parse_settings(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx, ngx_buf_t *b);
static ngx_int_t ngx_http_grpc_parse_ping(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx, ngx_buf_t *b);

static ngx_int_t ngx_http_grpc_send_settings_ack(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx);
static ngx_int_t ngx_http_grpc_send_ping_ack(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx);
static ngx_int_t ngx_http_grpc_send_window_update(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx);

static ngx_chain_t *ngx_http_grpc_get_buf(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx);
static ngx_http_grpc_ctx_t *ngx_http_grpc_get_ctx(ngx_http_request_t *r);
static ngx_int_t ngx_http_grpc_get_connection_data(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx, ngx_peer_connection_t *pc);
static void ngx_http_grpc_cleanup(void *data);

static void ngx_http_grpc_abort_request(ngx_http_request_t *r);
static void ngx_http_grpc_finalize_request(ngx_http_request_t *r, ngx_int_t rc);

static ngx_int_t ngx_http_grpc_internal_trailers_variable(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data);

static ngx_int_t ngx_http_grpc_add_variables(ngx_conf_t *cf);
static void *ngx_http_grpc_create_loc_conf(ngx_conf_t *cf);
static char *ngx_http_grpc_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child);
static ngx_int_t
ngx_http_grpc_init_headers(ngx_conf_t *cf, ngx_http_grpc_loc_conf_t *conf, ngx_http_grpc_headers_t *headers, ngx_keyval_t *default_headers);

static char *ngx_http_grpc_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
...
static ngx_conf_bitmask_t ngx_http_grpc_next_upstream_masks[] = {{ngx_string("error"),          NGX_HTTP_UPSTREAM_FT_ERROR},
...
static ngx_command_t ngx_http_grpc_commands[] = {
        ...
        {ngx_string("grpc_pass"), NGX_HTTP_LOC_CONF | NGX_HTTP_LIF_CONF | NGX_CONF_TAKE1, ngx_http_grpc_pass, NGX_HTTP_LOC_CONF_OFFSET, 0,
         NULL},

        {ngx_string("grpc_bind"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE12,
         ngx_http_upstream_bind_set_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_grpc_loc_conf_t, upstream.local), NULL},

        {ngx_string("grpc_socket_keepalive"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
         ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_grpc_loc_conf_t, upstream.socket_keepalive), NULL},

        {ngx_string("grpc_connect_timeout"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
         ngx_conf_set_msec_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_grpc_loc_conf_t, upstream.connect_timeout), NULL},

        {ngx_string("grpc_send_timeout"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
         ngx_conf_set_msec_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_grpc_loc_conf_t, upstream.send_timeout), NULL},

        {ngx_string("grpc_intercept_errors"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
         ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_grpc_loc_conf_t, upstream.intercept_errors), NULL},

        {ngx_string("grpc_buffer_size"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
         ngx_conf_set_size_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_grpc_loc_conf_t, upstream.buffer_size), NULL},

        {ngx_string("grpc_read_timeout"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
         ngx_conf_set_msec_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_grpc_loc_conf_t, upstream.read_timeout), NULL},

        {ngx_string("grpc_next_upstream"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_1MORE,
         ngx_conf_set_bitmask_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_grpc_loc_conf_t, upstream.next_upstream),
         &ngx_http_grpc_next_upstream_masks},

        {ngx_string("grpc_next_upstream_tries"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
         ngx_conf_set_num_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_grpc_loc_conf_t, upstream.next_upstream_tries), NULL},

        {ngx_string("grpc_next_upstream_timeout"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
         ngx_conf_set_msec_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_grpc_loc_conf_t, upstream.next_upstream_timeout), NULL},

        {ngx_string("grpc_set_header"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE2,
         ngx_conf_set_keyval_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_grpc_loc_conf_t, headers_source), NULL},

        {ngx_string("grpc_pass_header"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
         ngx_conf_set_str_array_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_grpc_loc_conf_t, upstream.pass_headers), NULL},

        {ngx_string("grpc_hide_header"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
         ngx_conf_set_str_array_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_grpc_loc_conf_t, upstream.hide_headers), NULL},

        {ngx_string("grpc_ignore_headers"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_1MORE,
         ngx_conf_set_bitmask_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_grpc_loc_conf_t, upstream.ignore_headers),
...
static ngx_http_module_t ngx_http_grpc_module_ctx = {ngx_http_grpc_add_variables,           /* preconfiguration */
...
ngx_module_t ngx_http_grpc_module = {NGX_MODULE_V1, &ngx_http_grpc_module_ctx,             /* module context */
...
static u_char ngx_http_grpc_connection_start[] = "PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n"         /* connection preface */
...
static ngx_keyval_t ngx_http_grpc_headers[] = {{ngx_string("Content-Length"),    ngx_string("$content_length")},
...
static ngx_str_t ngx_http_grpc_hide_headers[] = {ngx_string("Date"), ngx_string("Server"), ngx_string("X-Accel-Expires"),
                                                 ngx_string("X-Accel-Redirect"), ngx_string("X-Accel-Limit-Rate"),
                                                 ngx_string("X-Accel-Buffering"), ngx_string("X-Accel-Charset"), ngx_null_string};


static ngx_http_variable_t ngx_http_grpc_vars[] = {

        {ngx_string("grpc_internal_trailers"), NULL, ngx_http_grpc_internal_trailers_variable, 0,
         NGX_HTTP_VAR_NOCACHEABLE | NGX_HTTP_VAR_NOHASH, 0},

        ngx_http_null_variable};


static ngx_int_t ngx_http_grpc_handler(ngx_http_request_t *r) {
    ngx_int_t rc;
...
static ngx_int_t ngx_http_grpc_create_request(ngx_http_request_t *r) {
    ...
    len = sizeof(ngx_http_grpc_connection_start) - 1 + sizeof(ngx_http_grpc_frame_t);             /* headers frame */
    ...
    while (*(uintptr_t *) le.ip) {
        ...
        len += 1 + NGX_HTTP_V2_INT_OCTETS + key_len + NGX_HTTP_V2_INT_OCTETS + val_len;
    ...
    if (glcf->upstream.pass_request_headers) {
        ...
        for (i = 0; /* void */; i++) {
            ...
            if (ngx_hash_find(&glcf->headers.hash, header[i].hash, header[i].lowcase_key, header[i].key.len)) {
                continue;
            }
    ...
    len += sizeof(ngx_http_grpc_frame_t) * (len / NGX_HTTP_V2_DEFAULT_FRAME_SIZE);
    ...
    b->last = ngx_copy(b->last, ngx_http_grpc_connection_start, sizeof(ngx_http_grpc_connection_start) - 1);
    ...
    if (r->valid_unparsed_uri) {
        ...
        *b->last++ = ngx_http_v2_inc_indexed(NGX_HTTP_V2_PATH_INDEX);
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc header: \":path: %*s\"", p - val_tmp, val_tmp);
    ...
    if (!glcf->host_set) {
        *b->last++ = ngx_http_v2_inc_indexed(NGX_HTTP_V2_AUTHORITY_INDEX);
        b->last = ngx_http_v2_write_value(b->last, glcf->host.data, glcf->host.len, tmp);

        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc header: \":authority: %V\"", &glcf->host);
    }
    ...
    if (glcf->upstream.pass_request_headers) {
        ...
        for (i = 0; /* void */; i++) {
            ...
            *b->last++ = 0;
            ...
            b->last = ngx_http_v2_write_name(b->last, header[i].key.data, header[i].key.len, tmp);

            b->last = ngx_http_v2_write_value(b->last, header[i].value.data, header[i].value.len, tmp);
...
static ngx_int_t ngx_http_grpc_body_output_filter(void *data, ngx_chain_t *in) {
    ...
    if (!ctx->header_sent) {
        ...
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc output header");
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc output limit: %uz w:%z:%uz", limit, ctx->send_window,
    ...
    while (in && limit > 0) {
        ...
        ngx_log_debug7(NGX_LOG_DEBUG_EVENT, r->connection->log, 0, "grpc output in  l:%d f:%d %p, pos %p, size: %z "
    ...
    if (last) {
        ...
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc output last");
    ...
    *ll = NULL;
    ...
    ngx_chain_update_chains(r->pool, &ctx->free, &ctx->busy, &out, (ngx_buf_tag_t) &ngx_http_grpc_body_output_filter);
    ...
    if (ctx->done) {
        ...
        if (ctx->in == NULL && ctx->out == NULL && ctx->output_closed && !ctx->output_blocked && ctx->state == ngx_http_grpc_st_start) {
            u->keepalive = 1;
        }
...
static ngx_int_t ngx_http_grpc_process_header(ngx_http_request_t *r) {
    ...
    for (;;) {
        ...
        if (ctx->state < ngx_http_grpc_st_payload) {
            ...
            if (ctx->type == NGX_HTTP_V2_DATA_FRAME || (ctx->type == NGX_HTTP_V2_CONTINUATION_FRAME && !ctx->parsing_headers) ||
        ...
        if (ctx->type == NGX_HTTP_V2_PUSH_PROMISE_FRAME) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent unexpected push promise frame");
            return NGX_HTTP_UPSTREAM_INVALID_HEADER;
        }
        ...
        for (;;) {
            ...
            if (rc == NGX_OK) {
                ...
                ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc header: \"%V: %V\"", &ctx->name, &ctx->value);

                if (ctx->name.len && ctx->name.data[0] == ':') {

                    if (ctx->name.len != sizeof(":status") - 1 || ngx_strncmp(ctx->name.data, ":status", sizeof(":status") - 1) != 0) {
                        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent invalid header \"%V: %V\"", &ctx->name,
                                      &ctx->value);
                        return NGX_HTTP_UPSTREAM_INVALID_HEADER;
                    }

                    if (ctx->status) {
                        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent duplicate :status header");
                        return NGX_HTTP_UPSTREAM_INVALID_HEADER;
                    }
                    ...
                    if (status < NGX_HTTP_OK) {
                        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent unexpected :status \"%V\"", status_line);
                        return NGX_HTTP_UPSTREAM_INVALID_HEADER;
                    }
                    ...
                    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent no :status header");
                    return NGX_HTTP_UPSTREAM_INVALID_HEADER;
                }
            ...
            if (rc == NGX_HTTP_PARSE_HEADER_DONE) {
                ...
                ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc header done");

                if (ctx->end_stream) {
                    u->headers_in.content_length_n = 0;

                    if (ctx->in == NULL && ctx->out == NULL && ctx->output_closed && !ctx->output_blocked && b->last == b->pos) {
                        u->keepalive = 1;
                    }
                }
...
static ngx_int_t ngx_http_grpc_filter(void *data, ssize_t bytes) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc filter bytes:%z", bytes);
    ...
    for (;;) {
        ...
        if (ctx->state < ngx_http_grpc_st_payload) {
            ...
            if (rc == NGX_AGAIN) {
                ...
                if (ctx->done) {
                    ...
                    if (ctx->in == NULL && ctx->output_closed && !ctx->output_blocked && ctx->state == ngx_http_grpc_st_start) {
                        u->keepalive = 1;
                    }
            ...
            if ((ctx->type == NGX_HTTP_V2_CONTINUATION_FRAME && !ctx->parsing_headers) ||
                (ctx->type != NGX_HTTP_V2_CONTINUATION_FRAME && ctx->parsing_headers)) {
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent unexpected http2 frame: %d", ctx->type);
                return NGX_ERROR;
            }
            ...
            if (ctx->type == NGX_HTTP_V2_DATA_FRAME) {
                ...
                if (ctx->stream_id != ctx->id) {
                    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent data frame "
                                                                      "for unknown stream %ui", ctx->stream_id);
                    return NGX_ERROR;
                }

                if (ctx->rest > ctx->recv_window) {
                    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream violated stream flow control, "
                                                                      "received %uz data frame with window %uz", ctx->rest,
                                  ctx->recv_window);
                    return NGX_ERROR;
                }

                if (ctx->rest > ctx->connection->recv_window) {
                    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream violated connection flow control, "
                                                                      "received %uz data frame with window %uz", ctx->rest,
                                  ctx->connection->recv_window);
                    return NGX_ERROR;
                }
                ...
                if (ctx->connection->recv_window < NGX_HTTP_V2_MAX_WINDOW / 4 || ctx->recv_window < NGX_HTTP_V2_MAX_WINDOW / 4) {
            ...
            if (ctx->stream_id && ctx->done) {
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent frame for closed stream %ui", ctx->stream_id);
                return NGX_ERROR;
            }
        ...
        if (ctx->type == NGX_HTTP_V2_HEADERS_FRAME || ctx->type == NGX_HTTP_V2_CONTINUATION_FRAME) {
            for (;;) {
                ...
                if (rc == NGX_OK) {
                    ...
                    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc trailer: \"%V: %V\"", &ctx->name, &ctx->value);

                    if (ctx->name.len && ctx->name.data[0] == ':') {
                        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent invalid "
                                                                          "trailer \"%V: %V\"", &ctx->name, &ctx->value);
                        return NGX_ERROR;
                    }
                ...
                if (rc == NGX_HTTP_PARSE_HEADER_DONE) {
                    ...
                    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent trailer without "
                                                                      "end stream flag");
                    return NGX_ERROR;
                }
                ...
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent invalid trailer");
        ...
        if (ctx->flags & NGX_HTTP_V2_PADDED_FLAG) {
            ...
            if (ctx->rest == 0) {
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent too short http2 frame");
                return NGX_ERROR;
            }
            ...
            if (ctx->padding > ctx->rest) {
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent http2 frame with too long "
                                                                  "padding: %d in frame %uz", ctx->padding, ctx->rest);
                return NGX_ERROR;
            }
        ...
        *ll = cl;
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc output buf %p", buf->pos);
        ...
        switch (state) {
            ...
            case ngx_http_grpc_st_length_3:
                ...
                if (ctx->rest > NGX_HTTP_V2_DEFAULT_FRAME_SIZE) {
                    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent too large http2 frame: %uz", ctx->rest);
                    return NGX_ERROR;
                }
            ...
            case ngx_http_grpc_st_stream_id_4:
                ...
                ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc frame: %d, len: %uz, f:%d, i:%ui", ctx->type, ctx->rest,
                               ctx->flags, ctx->stream_id);
...
static ngx_int_t ngx_http_grpc_parse_header(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx, ngx_buf_t *b) {
    ...
    if (state == sw_start) {
        ...
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc parse header: start");

        if (ctx->type == NGX_HTTP_V2_HEADERS_FRAME) {
            ctx->parsing_headers = 1;
            ctx->fragment_state = 0;

            min = (ctx->flags & NGX_HTTP_V2_PADDED_FLAG ? 1 : 0) + (ctx->flags & NGX_HTTP_V2_PRIORITY_FLAG ? 5 : 0);

            if (ctx->rest < min) {
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent headers frame "
                                                                  "with invalid length: %uz", ctx->rest);
                return NGX_ERROR;
            }
    ...
    if (state == sw_padding) {
        ...
        if (ctx->flags & NGX_HTTP_V2_END_HEADERS_FLAG) {
            ...
            if (ctx->fragment_state) {
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent truncated http2 header");
                return NGX_ERROR;
            }
    ...
    for (p = b->pos; p < last; p++) {
        ...
        switch (state) {
            ...
            case sw_start:
                ...
                if ((ch & 0x80) == 0x80) {
                    ...
                    if (index == 0 || index > 61) {
                        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent invalid http2 "
                                                                          "table index: %ui", index);
                        return NGX_ERROR;
                    }

                    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc indexed header: %ui", index);
                    ...
                    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc literal header: %ui", index);
                    ...
                    if (size_update > 0) {
                        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent invalid http2 "
                                                                          "dynamic table size update: %ui", size_update);
                        return NGX_ERROR;
                    }

                    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc table size update: %ui", size_update);
                    ...
                    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc literal header never indexed: %ui", index);
                    ...
                    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc literal header without indexing: %ui", index);
            ...
            case sw_index:
                ...
                if (ch & 0x80) {
                    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent http2 table index "
                ...
                ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc header index: %ui", ctx->index);
            ...
            case sw_name_length:
                ...
                if (ctx->field_length == 0) {
                    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent zero http2 "
                                                                      "header name length");
                    return NGX_ERROR;
                }
            ...
            case sw_name_length_4:
                ...
                if (ch & 0x80) {
                    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent too large http2 "
                                                                      "header name length");
                    return NGX_ERROR;
                }
            ...
            case sw_name:
                ctx->name.len = ctx->field_huffman ? ctx->field_length * 8 / 5 : ctx->field_length;
            ...
            case sw_name_bytes:
                ...
                ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc name: len:%uz h:%d last:%uz, rest:%uz", ctx->field_length,
                               ctx->field_huffman, last - p, ctx->rest - (p - b->pos));

                size = ngx_min(last - p, (ssize_t) ctx->field_rest);
                ctx->field_rest -= size;

                if (ctx->field_huffman) {
                    if (ngx_http_v2_huff_decode(&ctx->field_state, p, size, &ctx->field_end, ctx->field_rest == 0, r->connection->log) !=
                        NGX_OK) {
                        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent invalid encoded header");
                        return NGX_ERROR;
                    }
            ...
            case sw_value:
                ctx->value.len = ctx->field_huffman ? ctx->field_length * 8 / 5 : ctx->field_length;
            ...
            case sw_value_bytes:
                ...
                ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc value: len:%uz h:%d last:%uz, rest:%uz", ctx->field_length,
                               ctx->field_huffman, last - p, ctx->rest - (p - b->pos));
        ...
        if (!ctx->index) {
            if (ngx_http_grpc_validate_header_name(r, &ctx->name) != NGX_OK) {
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent invalid header: \"%V: %V\"", &ctx->name, &ctx->value);
                return NGX_ERROR;
            }
        }
...
static ngx_int_t ngx_http_grpc_parse_rst_stream(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx, ngx_buf_t *b) {
    u_char ch, *p, *last;
    enum {
        sw_start = 0, sw_error_2, sw_error_3, sw_error_4
    } state;
    ...
    if (state == sw_start) {
        if (ctx->rest != 4) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent rst stream frame "
                                                              "with invalid length: %uz", ctx->rest);
            return NGX_ERROR;
        }
    }
    ...
    for (p = b->pos; p < last; p++) {
        ...
        switch (state) {
            ...
            case sw_error_4:
                ...
                ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc error: %ui", ctx->error);
...
static ngx_int_t ngx_http_grpc_parse_goaway(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx, ngx_buf_t *b) {
    u_char ch, *p, *last;
    enum {
        sw_start = 0, sw_last_stream_id_2, sw_last_stream_id_3, sw_last_stream_id_4, sw_error, sw_error_2, sw_error_3, sw_error_4, sw_debug
    } state;
    ...
    if (state == sw_start) {
        ...
        if (ctx->stream_id) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent goaway frame "
                                                              "with non-zero stream id: %ui", ctx->stream_id);
            return NGX_ERROR;
        }
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc goaway: %ui, stream %ui", ctx->error, ctx->stream_id);
...
static ngx_int_t ngx_http_grpc_parse_window_update(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx, ngx_buf_t *b) {
    u_char ch, *p, *last;
    enum {
        sw_start = 0, sw_size_2, sw_size_3, sw_size_4
    } state;
    ...
    if (state == sw_start) {
        if (ctx->rest != 4) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent window update frame "
                                                              "with invalid length: %uz", ctx->rest);
            return NGX_ERROR;
        }
    }
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc window update: %ui", ctx->window_update);

    if (ctx->stream_id) {

        if (ctx->window_update > (size_t) NGX_HTTP_V2_MAX_WINDOW - ctx->send_window) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent too large window update");
            return NGX_ERROR;
        }
        ...
        if (ctx->window_update > NGX_HTTP_V2_MAX_WINDOW - ctx->connection->send_window) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent too large window update");
            return NGX_ERROR;
        }
...
static ngx_int_t ngx_http_grpc_parse_settings(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx, ngx_buf_t *b) {
    ...
    enum {
        sw_start = 0, sw_id, sw_id_2, sw_value, sw_value_2, sw_value_3, sw_value_4
    } state;
    ...
    if (state == sw_start) {
        ...
        if (ctx->stream_id) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent settings frame "
                                                              "with non-zero stream id: %ui", ctx->stream_id);
            return NGX_ERROR;
        }

        if (ctx->flags & NGX_HTTP_V2_ACK_FLAG) {
            ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc settings ack");

            if (ctx->rest != 0) {
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent settings frame "
                                                                  "with ack flag and non-zero length: %uz", ctx->rest);
                return NGX_ERROR;
            }
        ...
        if (ctx->free == NULL && ctx->settings++ > 1000) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent too many settings frames");
            return NGX_ERROR;
        }
    }
    ...
    for (p = b->pos; p < last; p++) {
        ...
        switch (state) {
            ...
            case sw_value_4:
                ...
                ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc setting: %ui %ui", ctx->setting_id, ctx->setting_value);
                ...
                if (ctx->setting_id == 0x04) {
                    ...
                    if (ctx->setting_value > NGX_HTTP_V2_MAX_WINDOW) {
                        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent settings frame "
                                                                          "with too large initial window size: %ui", ctx->setting_value);
                        return NGX_ERROR;
                    }
                    ...
                    if (ctx->send_window > 0 && window_update > (ssize_t) NGX_HTTP_V2_MAX_WINDOW - ctx->send_window) {
                        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent settings frame "
                                                                          "with too large initial window size: %ui", ctx->setting_value);
                        return NGX_ERROR;
                    }
...
static ngx_int_t ngx_http_grpc_parse_ping(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx, ngx_buf_t *b) {
    u_char ch, *p, *last;
    enum {
        sw_start = 0, sw_data_2, sw_data_3, sw_data_4, sw_data_5, sw_data_6, sw_data_7, sw_data_8
    } state;
    ...
    if (state == sw_start) {
        ...
        if (ctx->stream_id) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent ping frame "
                                                              "with non-zero stream id: %ui", ctx->stream_id);
            return NGX_ERROR;
        }
        ...
        if (ctx->flags & NGX_HTTP_V2_ACK_FLAG) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent ping frame with ack flag");
            return NGX_ERROR;
        }

        if (ctx->free == NULL && ctx->pings++ > 1000) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent too many ping frames");
            return NGX_ERROR;
        }
    }
    ...
    for (p = b->pos; p < last; p++) {
        ...
        if (state < sw_data_8) {
            ...
            ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc ping");
        }
    }
...
static ngx_int_t ngx_http_grpc_send_settings_ack(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx) {
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc send settings ack");
...
static ngx_int_t ngx_http_grpc_send_ping_ack(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx) {
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc send ping ack");
...
static ngx_int_t ngx_http_grpc_send_window_update(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx) {
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "grpc send window update: %uz %uz", ctx->connection->recv_window,
                   ctx->recv_window);
...
static ngx_int_t ngx_http_grpc_get_connection_data(ngx_http_request_t *r, ngx_http_grpc_ctx_t *ctx, ngx_peer_connection_t *pc) {
    ...
    if (pc->cached) {
        ...
        if (ctx->connection == NULL) {
            ngx_log_error(NGX_LOG_ERR, c->log, 0, "no connection data found for "
...
static void ngx_http_grpc_abort_request(ngx_http_request_t *r) {
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "abort grpc request");
    return;
}


static void ngx_http_grpc_finalize_request(ngx_http_request_t *r, ngx_int_t rc) {
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "finalize grpc request");
    return;
}
...
static ngx_int_t ngx_http_grpc_internal_trailers_variable(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ...
    if (ngx_strlcasestrn(te->value.data, te->value.data + te->value.len, (u_char *) "trailers", 8 - 1) == NULL) {
        v->not_found = 1;
        return NGX_OK;
    }
...
static char *ngx_http_grpc_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child) {
    ...
    if (ngx_http_upstream_hide_headers_hash(cf, &conf->upstream, &prev->upstream, ngx_http_grpc_hide_headers, &hash) != NGX_OK) {
        return NGX_CONF_ERROR;
    }
    ...
    rc = ngx_http_grpc_init_headers(cf, conf, &conf->headers, ngx_http_grpc_headers);
    if (rc != NGX_OK) {
        return NGX_CONF_ERROR;
    }
...
static ngx_int_t ngx_http_grpc_init_headers(ngx_conf_t *cf, ngx_http_grpc_loc_conf_t *conf, ngx_http_grpc_headers_t *headers,
    ...
    if (conf->headers_source) {
        ...
        for (i = 0; i < conf->headers_source->nelts; i++) {
            ...
            if (src[i].key.len == 4 && ngx_strncasecmp(src[i].key.data, (u_char *) "Host", 4) == 0) {
                conf->host_set = 1;
            }
...
static char *ngx_http_grpc_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    if (ngx_strncasecmp(url->data, (u_char *) "grpc://", 7) == 0) {
        ...
        add = 8;
#else
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "grpcs protocol requires SSL support");
        return NGX_CONF_ERROR;
#endif
