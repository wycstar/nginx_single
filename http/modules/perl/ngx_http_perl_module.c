...
static char *ngx_http_perl_init_interpreter(ngx_conf_t *cf, ngx_http_perl_main_conf_t *pmcf);
static PerlInterpreter *ngx_http_perl_create_interpreter(ngx_conf_t *cf, ngx_http_perl_main_conf_t *pmcf);
static ngx_int_t ngx_http_perl_run_requires(pTHX_ ngx_array_t
*requires,
ngx_log_t *log
);
static ngx_int_t ngx_http_perl_call_handler(pTHX_ ngx_http_request_t
*r,
HV *nginx, SV
*sub,
SV **args, ngx_str_t
...
static void ngx_http_perl_eval_anon_sub(pTHX_ ngx_str_t
*handler,
SV **sv
);
...
static ngx_command_t ngx_http_perl_commands[] = {
        ...
        {ngx_string("perl_modules"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE1, ngx_conf_set_str_array_slot, NGX_HTTP_MAIN_CONF_OFFSET,
         offsetof(ngx_http_perl_main_conf_t, modules), NULL},

        {ngx_string("perl_require"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE1, ngx_conf_set_str_array_slot, NGX_HTTP_MAIN_CONF_OFFSET,
         offsetof(ngx_http_perl_main_conf_t, requires), NULL},

        {ngx_string("perl"), NGX_HTTP_LOC_CONF | NGX_HTTP_LMT_CONF | NGX_CONF_TAKE1, ngx_http_perl, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

        {ngx_string("perl_set"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE2, ngx_http_perl_set, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

        ngx_null_command};


static ngx_http_module_t ngx_http_perl_module_ctx = {ngx_http_perl_preconfiguration,        /* preconfiguration */
...
ngx_module_t ngx_http_perl_module = {NGX_MODULE_V1, &ngx_http_perl_module_ctx,             /* module context */
...
static ngx_http_ssi_param_t ngx_http_perl_ssi_params[] = {{ngx_string("sub"), NGX_HTTP_PERL_SSI_SUB, 1, 0},
                                                          {ngx_string("arg"), NGX_HTTP_PERL_SSI_ARG, 0, 1},
                                                          {ngx_null_string, 0,                       0, 0}};

static ngx_http_ssi_command_t ngx_http_perl_ssi_command = {ngx_string("perl"), ngx_http_perl_ssi, ngx_http_perl_ssi_params, 0, 0, 1};
...
static void ngx_http_perl_xs_init(pTHX) {
...
static ngx_int_t ngx_http_perl_handler(ngx_http_request_t *r) {
    r->main->count++;
...
void ngx_http_perl_handle_request(ngx_http_request_t *r) {
...

    {
        ...
        rc = ngx_http_perl_call_handler(aTHX_
        r, pmcf->nginx, sub, NULL, handler, NULL);

    }

    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "perl handler done: %i", rc);
...
void ngx_http_perl_sleep_handler(ngx_http_request_t *r) {
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "perl sleep handler");
...
static ngx_int_t ngx_http_perl_variable(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "perl variable handler");
...

    {
        ...
        rc = ngx_http_perl_call_handler(aTHX_
        r, pmcf->nginx, pv->sub, NULL, &pv->handler, &value);
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "perl variable done");
...
static ngx_int_t ngx_http_perl_ssi(ngx_http_request_t *r, ngx_http_ssi_ctx_t *ssi_ctx, ngx_str_t **params) {
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "perl ssi handler");
...

    {
        ...
        rc = ngx_http_perl_call_handler(aTHX_
        r, pmcf->nginx, sv, asv, handler, NULL);
...
static PerlInterpreter *ngx_http_perl_create_interpreter(ngx_conf_t *cf, ngx_http_perl_main_conf_t *pmcf) {
...

    {
        ...
        if (ngx_strcmp(ver, NGINX_VERSION) != 0) {
            ngx_log_error(NGX_LOG_ALERT, cf->log, 0, "version "
                    NGINX_VERSION
...
if (requires == NGX_CONF_UNSET_PTR) { return NGX_OK; }
...
ngx_log_error(NGX_LOG_EMERG, log, 0, "require_pv(\"%s\") failed: \"%*s\"", script[i].data, len + 1, err);
...
if (args) { EXTEND(sp, (intptr_t)
args[0]);

for (
i = 1;
i <= (uintptr_t) args[0]; i++) { PUSHs (sv_2mortal(args[i])); }}
...
if (n) { if (rv == NULL) { status = POPi;

ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log, 0, "call_sv: %d", status);

} else { line = SvPVx(POPs, n_a);
rv->
len = n_a;

rv->
data = ngx_pnalloc(r->pool, n_a);
if (rv->data == NULL) { return NGX_ERROR; }
...
ngx_log_error(NGX_LOG_ERR, c->log, 0, "call_sv(\"%V\") failed: \"%*s\"", handler, len + 1, err);

if (rv) { return NGX_ERROR; }

return NGX_HTTP_INTERNAL_SERVER_ERROR; }

if (n != 1) {
ngx_log_error(NGX_LOG_ALERT, c->log, 0, "call_sv(\"%V\") returned %d results", handler, n);
status = NGX_OK; }
...
static char *ngx_http_perl_set(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
...

    {
        ...
        if (pv->sub == &PL_sv_undef) {
            ngx_conf_log_error(NGX_LOG_ERR, cf, 0, "eval_pv(\"%V\") failed", &value[2]);
            return NGX_CONF_ERROR;
        }
