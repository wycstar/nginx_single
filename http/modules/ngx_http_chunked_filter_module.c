...
static ngx_http_module_t ngx_http_chunked_filter_module_ctx = {NULL,                                  /* preconfiguration */
...
ngx_module_t ngx_http_chunked_filter_module = {NGX_MODULE_V1, &ngx_http_chunked_filter_module_ctx,   /* module context */
...
static ngx_int_t ngx_http_chunked_header_filter(ngx_http_request_t *r) {
    ...
    if (r->headers_out.status == NGX_HTTP_NOT_MODIFIED || r->headers_out.status == NGX_HTTP_NO_CONTENT ||
        r->headers_out.status < NGX_HTTP_OK || r != r->main || r->method == NGX_HTTP_HEAD) {
...
static ngx_chain_t *ngx_http_chunked_create_trailers(ngx_http_request_t *r, ngx_http_chunked_filter_ctx_t *ctx) {
    ...
    *b->last++ = LF;
    ...
    for (i = 0; /* void */; i++) {
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http trailer: \"%V: %V\"", &header[i].key, &header[i].value);
