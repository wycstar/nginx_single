...
static ngx_http_realip_ctx_t *ngx_http_realip_get_module_ctx(ngx_http_request_t *r);


static ngx_int_t ngx_http_realip_remote_addr_variable(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_realip_remote_port_variable(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data);


static ngx_command_t ngx_http_realip_commands[] = {

        {ngx_string("set_real_ip_from"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1, ngx_http_realip_from,
         NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

        {ngx_string("real_ip_header"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1, ngx_http_realip,
         NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

        {ngx_string("real_ip_recursive"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
         ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_realip_loc_conf_t, recursive), NULL},

        ngx_null_command};


static ngx_http_module_t ngx_http_realip_module_ctx = {ngx_http_realip_add_variables,         /* preconfiguration */
...
ngx_module_t ngx_http_realip_module = {NGX_MODULE_V1, &ngx_http_realip_module_ctx,           /* module context */
...
static ngx_http_variable_t ngx_http_realip_vars[] = {
        ...
        {ngx_string("realip_remote_addr"), NULL, ngx_http_realip_remote_addr_variable, 0, 0, 0},

        {ngx_string("realip_remote_port"), NULL, ngx_http_realip_remote_port_variable, 0, 0, 0},
...
static ngx_int_t ngx_http_realip_handler(ngx_http_request_t *r) {
    ...
    switch (rlcf->type) {
        ...
        default: /* NGX_HTTP_REALIP_HEADER */
            ...
            for (i = 0; /* void */ ; i++) {
                ...
                if (hash == header[i].hash && len == header[i].key.len && ngx_strncmp(p, header[i].lowcase_key, len) == 0) {
    ...
    if (ngx_http_get_forwarded_addr(r, &addr, xfwd, value, rlcf->from, rlcf->recursive) != NGX_DECLINED) {
...
static char *ngx_http_realip_from(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    if (ngx_inet_resolve_host(cf->pool, &u) != NGX_OK) {
        if (u.err) {
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "%s in set_real_ip_from \"%V\"", u.err, &u.host);
        }
