...
static ngx_int_t ngx_http_add_multi_header_lines(ngx_http_request_t *r, ngx_http_header_val_t *hv, ngx_str_t *value);
...
static ngx_http_set_header_t ngx_http_set_headers[] = {
        ...
        {ngx_string("Cache-Control"), offsetof(ngx_http_headers_out_t, cache_control), ngx_http_add_multi_header_lines},

        {ngx_string("Link"),          offsetof(ngx_http_headers_out_t, link),          ngx_http_add_multi_header_lines},
...
static ngx_command_t ngx_http_headers_filter_commands[] = {
        ...
        {ngx_string("expires"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_HTTP_LIF_CONF | NGX_CONF_TAKE12,
         ngx_http_headers_expires, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

        {ngx_string("add_header"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_HTTP_LIF_CONF | NGX_CONF_TAKE23,
         ngx_http_headers_add, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_headers_conf_t, headers), NULL},

        {ngx_string("add_trailer"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_HTTP_LIF_CONF | NGX_CONF_TAKE23,
         ngx_http_headers_add, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_headers_conf_t, trailers), NULL},

        ngx_null_command};


static ngx_http_module_t ngx_http_headers_filter_module_ctx = {NULL,                                  /* preconfiguration */
...
ngx_module_t ngx_http_headers_filter_module = {NGX_MODULE_V1, &ngx_http_headers_filter_module_ctx,   /* module context */
...
static ngx_int_t ngx_http_headers_filter(ngx_http_request_t *r) {
    ...
    if (conf->expires == NGX_HTTP_EXPIRES_OFF && conf->headers == NULL && conf->trailers == NULL) {
        return ngx_http_next_header_filter(r);
    }
...
static char *ngx_http_headers_expires(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    if (cv.lengths != NULL) {
        ...
        hcf->expires_value = ngx_palloc(cf->pool, sizeof(ngx_http_complex_value_t));
