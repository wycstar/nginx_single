...
static ngx_command_t ngx_http_try_files_commands[] = {
        ...
        {ngx_string("try_files"), NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_2MORE, ngx_http_try_files, NGX_HTTP_LOC_CONF_OFFSET, 0,
         NULL},

        ngx_null_command};


static ngx_http_module_t ngx_http_try_files_module_ctx = {NULL,                                  /* preconfiguration */
...
ngx_module_t ngx_http_try_files_module = {NGX_MODULE_V1, &ngx_http_try_files_module_ctx,        /* module context */
...
static ngx_int_t ngx_http_try_files_handler(ngx_http_request_t *r) {
    ...
    for (;;) {
        ...
        ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "trying to use %s: \"%s\" \"%s\"", test_dir ? "dir" : "file", name,
                       path.data);
