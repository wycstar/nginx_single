...
static ngx_command_t ngx_http_sub_filter_commands[] = {
        ...
        {ngx_string("sub_filter"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE2, ngx_http_sub_filter,
         NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

        {ngx_string("sub_filter_types"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_1MORE, ngx_http_types_slot,
         NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_sub_loc_conf_t, types_keys), &ngx_http_html_default_types[0]},

        {ngx_string("sub_filter_once"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG, ngx_conf_set_flag_slot,
         NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_sub_loc_conf_t, once), NULL},
...
static ngx_http_module_t ngx_http_sub_filter_module_ctx = {NULL,                                  /* preconfiguration */
...
ngx_module_t ngx_http_sub_filter_module = {NGX_MODULE_V1, &ngx_http_sub_filter_module_ctx,       /* module context */
...
static ngx_int_t ngx_http_sub_header_filter(ngx_http_request_t *r) {
    ...
    if (slcf->dynamic == 0) {
        ...
        ngx_http_sub_init_tables(ctx->tables, ctx->matches->elts, ctx->matches->nelts);
    }
...
static ngx_int_t ngx_http_sub_body_filter(ngx_http_request_t *r, ngx_chain_t *in) {
    ...
    while (ctx->in || ctx->buf) {
        ...
        while (ctx->pos < ctx->buf->last) {
            ...
            ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "parse: %i, looked: \"%V\" %p-%p", rc, &ctx->looked, ctx->copy_start,
                           ctx->copy_end);
            ...
            if (ctx->sub == NULL) {
                ctx->sub = ngx_pcalloc(r->pool, sizeof(ngx_str_t) * ctx->matches->nelts);
                if (ctx->sub == NULL) {
                    return NGX_ERROR;
                }
            }
...
static char *ngx_http_sub_merge_conf(ngx_conf_t *cf, void *parent, void *child) {
    ...
    if (conf->pairs && conf->dynamic == 0 && conf->tables == NULL) {
        ...
        ngx_http_sub_init_tables(conf->tables, conf->matches->elts, conf->matches->nelts);
    }
