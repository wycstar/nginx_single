...
static char *ngx_http_geo_cidr_add(ngx_conf_t *cf, ngx_http_geo_conf_ctx_t *ctx, ngx_cidr_t *cidr, ngx_str_t *value, ngx_str_t *net);
...
static ngx_http_module_t ngx_http_geo_module_ctx = {NULL,                                  /* preconfiguration */
...
ngx_module_t ngx_http_geo_module = {NGX_MODULE_V1, &ngx_http_geo_module_ctx,              /* module context */
...
static ngx_int_t ngx_http_geo_addr(ngx_http_request_t *r, ngx_http_geo_ctx_t *ctx, ngx_addr_t *addr) {
    ...
    if (xfwd->nelts > 0 && ctx->proxies != NULL) {
        (void) ngx_http_get_forwarded_addr(r, addr, xfwd, NULL, ctx->proxies, ctx->proxy_recursive);
    }
...
static char *ngx_http_geo_range(ngx_conf_t *cf, ngx_http_geo_conf_ctx_t *ctx, ngx_str_t *value) {
    ...
    if (ctx->high.low == NULL) {
        ctx->high.low = ngx_pcalloc(ctx->pool, 0x10000 * sizeof(ngx_http_geo_range_t *));
        if (ctx->high.low == NULL) {
            return NGX_CONF_ERROR;
        }
    }
    ...
    for (n = start; n <= end; n = (n + 0x10000) & 0xffff0000) {
        ...
        while (i) {
            ...
            if (s > (ngx_uint_t) range[i].end) {
                ...
                ngx_memmove(&range[i + 2], &range[i + 1], (a->nelts - 2 - i) * sizeof(ngx_http_geo_range_t));
            ...
            if (s > (ngx_uint_t) range[i].start && e < (ngx_uint_t) range[i].end) {
                ...
                ngx_memmove(&range[i + 3], &range[i + 1], (a->nelts - 3 - i) * sizeof(ngx_http_geo_range_t));
            ...
            if (s == (ngx_uint_t) range[i].start && e < (ngx_uint_t) range[i].end) {
                ...
                ngx_memmove(&range[i + 1], &range[i], (a->nelts - 1 - i) * sizeof(ngx_http_geo_range_t));
        ...
        ngx_memmove(&range[1], &range[0], (a->nelts - 1) * sizeof(ngx_http_geo_range_t));
...
static ngx_uint_t ngx_http_geo_delete_range(ngx_conf_t *cf, ngx_http_geo_conf_ctx_t *ctx, in_addr_t start, in_addr_t end) {
    ...
    for (n = start; n <= end; n = (n + 0x10000) & 0xffff0000) {
        ...
        for (i = 0; i < a->nelts; i++) {
            ...
            if (s == (ngx_uint_t) range[i].start && e == (ngx_uint_t) range[i].end) {
                ngx_memmove(&range[i], &range[i + 1], (a->nelts - 1 - i) * sizeof(ngx_http_geo_range_t));
...
static char *ngx_http_geo_cidr_add(ngx_conf_t *cf, ngx_http_geo_conf_ctx_t *ctx, ngx_cidr_t *cidr, ngx_str_t *value, ngx_str_t *net) {
    ...
    switch (cidr->family) {
        ...
        case AF_INET6:
            ...
            old = (ngx_http_variable_value_t *) ngx_radix128tree_find(ctx->tree6, cidr->u.in6.addr.s6_addr);
        ...
        default: /* AF_INET */
            ...
            old = (ngx_http_variable_value_t *) ngx_radix32tree_find(ctx->tree, cidr->u.in.addr);
...
static ngx_http_variable_value_t *ngx_http_geo_value(ngx_conf_t *cf, ngx_http_geo_conf_ctx_t *ctx, ngx_str_t *value) {
    ...
    gvvn = (ngx_http_geo_variable_value_node_t *) ngx_str_rbtree_lookup(&ctx->rbtree, value, hash);
    ...
    gvvn = ngx_palloc(ctx->temp_pool, sizeof(ngx_http_geo_variable_value_node_t));
    if (gvvn == NULL) {
        return NULL;
    }
...
static void ngx_http_geo_create_binary_base(ngx_http_geo_conf_ctx_t *ctx) {
    ...
    p = ngx_http_geo_copy_values(fm.addr, p, ctx->rbtree.root, ctx->rbtree.sentinel);
