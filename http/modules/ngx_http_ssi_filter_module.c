...
static ngx_str_t *ngx_http_ssi_get_variable(ngx_http_request_t *r, ngx_str_t *name, ngx_uint_t key);
...
static ngx_int_t ngx_http_ssi_regex_match(ngx_http_request_t *r, ngx_str_t *pattern, ngx_str_t *str);
...
static ngx_int_t ngx_http_ssi_echo(ngx_http_request_t *r, ngx_http_ssi_ctx_t *ctx, ngx_str_t **params);
static ngx_int_t ngx_http_ssi_config(ngx_http_request_t *r, ngx_http_ssi_ctx_t *ctx, ngx_str_t **params);
static ngx_int_t ngx_http_ssi_set(ngx_http_request_t *r, ngx_http_ssi_ctx_t *ctx, ngx_str_t **params);
...
static ngx_command_t ngx_http_ssi_filter_commands[] = {
        ...
        {ngx_string("ssi"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_HTTP_LIF_CONF | NGX_CONF_FLAG,
         ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_ssi_loc_conf_t, enable), NULL},

        {ngx_string("ssi_silent_errors"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
         ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_ssi_loc_conf_t, silent_errors), NULL},

        {ngx_string("ssi_ignore_recycled_buffers"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
         ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_ssi_loc_conf_t, ignore_recycled_buffers), NULL},

        {ngx_string("ssi_min_file_chunk"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
         ngx_conf_set_size_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_ssi_loc_conf_t, min_file_chunk), NULL},

        {ngx_string("ssi_value_length"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
         ngx_conf_set_size_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_ssi_loc_conf_t, value_len), NULL},

        {ngx_string("ssi_types"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_1MORE, ngx_http_types_slot,
         NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_ssi_loc_conf_t, types_keys), &ngx_http_html_default_types[0]},

        {ngx_string("ssi_last_modified"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
         ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_ssi_loc_conf_t, last_modified), NULL},

        ngx_null_command};


static ngx_http_module_t ngx_http_ssi_filter_module_ctx = {ngx_http_ssi_preconfiguration,         /* preconfiguration */
...
ngx_module_t ngx_http_ssi_filter_module = {NGX_MODULE_V1, &ngx_http_ssi_filter_module_ctx,       /* module context */
...
static ngx_http_ssi_param_t ngx_http_ssi_include_params[] = {{ngx_string("virtual"), NGX_HTTP_SSI_INCLUDE_VIRTUAL, 0, 0},
...
static ngx_http_ssi_param_t ngx_http_ssi_echo_params[] = {{ngx_string("var"),      NGX_HTTP_SSI_ECHO_VAR,      1, 0},
                                                          {ngx_string("default"),  NGX_HTTP_SSI_ECHO_DEFAULT,  0, 0},
                                                          {ngx_string("encoding"), NGX_HTTP_SSI_ECHO_ENCODING, 0, 0},
                                                          {ngx_null_string, 0,                                 0, 0}};


static ngx_http_ssi_param_t ngx_http_ssi_config_params[] = {{ngx_string("errmsg"),  NGX_HTTP_SSI_CONFIG_ERRMSG,  0, 0},
                                                            {ngx_string("timefmt"), NGX_HTTP_SSI_CONFIG_TIMEFMT, 0, 0},
                                                            {ngx_null_string, 0,                                 0, 0}};


static ngx_http_ssi_param_t ngx_http_ssi_set_params[] = {{ngx_string("var"),   NGX_HTTP_SSI_SET_VAR,   1, 0},
                                                         {ngx_string("value"), NGX_HTTP_SSI_SET_VALUE, 1, 0},
                                                         {ngx_null_string, 0,                          0, 0}};


static ngx_http_ssi_param_t ngx_http_ssi_if_params[] = {{ngx_string("expr"), NGX_HTTP_SSI_IF_EXPR, 1, 0},
                                                        {ngx_null_string, 0,                       0, 0}};


static ngx_http_ssi_param_t ngx_http_ssi_block_params[] = {{ngx_string("name"), NGX_HTTP_SSI_BLOCK_NAME, 1, 0},
                                                           {ngx_null_string, 0,                          0, 0}};


static ngx_http_ssi_param_t ngx_http_ssi_no_params[] = {{ngx_null_string, 0, 0, 0}};


static ngx_http_ssi_command_t ngx_http_ssi_commands[] = {{ngx_string(
                                                                  "include"),     ngx_http_ssi_include,  ngx_http_ssi_include_params, 0,                 0, 1},
                                                         {ngx_string(
                                                                  "echo"),        ngx_http_ssi_echo,     ngx_http_ssi_echo_params,    0,                 0, 0},
                                                         {ngx_string(
                                                                  "config"),      ngx_http_ssi_config,   ngx_http_ssi_config_params,  0,                 0, 0},
                                                         {ngx_string(
                                                         ...
                                                         {ngx_string(
                                                                  "elif"),        ngx_http_ssi_if,       ngx_http_ssi_if_params, NGX_HTTP_SSI_COND_IF,   0, 0},
                                                         {ngx_string(
                                                                  "else"),        ngx_http_ssi_else,     ngx_http_ssi_no_params, NGX_HTTP_SSI_COND_IF,   0, 0},
                                                         {ngx_string(
                                                                  "endif"),       ngx_http_ssi_endif,    ngx_http_ssi_no_params, NGX_HTTP_SSI_COND_ELSE, 0, 0},

                                                         {ngx_string(
                                                                  "block"),       ngx_http_ssi_block,    ngx_http_ssi_block_params,   0,                 0, 0},
                                                         {ngx_string(
                                                                  "endblock"),    ngx_http_ssi_endblock, ngx_http_ssi_no_params,      0,                 1, 0},
...
static ngx_int_t ngx_http_ssi_header_filter(ngx_http_request_t *r) {
    ...
    if (!slcf->enable || r->headers_out.content_length_n == 0 || ngx_http_test_content_type(r, &slcf->types) == NULL) {
    ...
    ngx_str_set(&ctx->errmsg, "[an error occurred while processing the directive]");
...
static ngx_int_t ngx_http_ssi_body_filter(ngx_http_request_t *r, ngx_chain_t *in) {
    ...
    if (ctx->wait) {
        ...
        if (r != r->connection->data) {
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http ssi filter wait \"%V?%V\" non-active", &ctx->wait->uri,
                           &ctx->wait->args);
        ...
        if (ctx->wait->done) {
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http ssi filter wait \"%V?%V\" done", &ctx->wait->uri,
                           &ctx->wait->args);

            ctx->wait = NULL;

        } else {
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http ssi filter wait \"%V?%V\"", &ctx->wait->uri, &ctx->wait->args);
    ...
    while (ctx->in || ctx->buf) {
        ...
        while (ctx->pos < ctx->buf->last) {
            ...
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "saved: %uz state: %ui", ctx->saved, ctx->state);

            rc = ngx_http_ssi_parse(r, ctx);

            ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "parse: %i, looked: %uz %p-%p", rc, ctx->looked, ctx->copy_start,
                           ctx->copy_end);
            ...
            if (ctx->copy_start != ctx->copy_end) {
                ...
                if (ctx->output) {
                    ...
                    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "saved: %uz", ctx->saved);
                    ...
                    *ctx->last_out = cl;
                    ...
                    if (ctx->block && ctx->saved + (ctx->copy_end - ctx->copy_start)) {
                        ...
                        if (ctx->saved) {
                            b->last = ngx_cpymem(b->pos, ngx_http_ssi_string, ctx->saved);
                        }

                        b->last = ngx_cpymem(b->last, ctx->copy_start, ctx->copy_end - ctx->copy_start);
            ...
            if (rc == NGX_OK) {
                ...
                smcf = ngx_http_get_module_main_conf(r, ngx_http_ssi_filter_module);

                cmd = ngx_hash_find(&smcf->hash, ctx->key, ctx->command.data, ctx->command.len);
                ...
                if (!ctx->output && !cmd->block) {
                    ...
                    if (ctx->block) {
                        ...
                        for (i = 0; i < ctx->params.nelts; i++) {
                            len += 1 + param[i].key.len + 2 + param[i].value.len + 1;
                        }
                        ...
                        *b->last++ = '#';
                        ...
                        b->last = ngx_cpymem(b->last, ctx->command.data, ctx->command.len);

                        for (i = 0; i < ctx->params.nelts; i++) {
                            *b->last++ = ' ';
                            b->last = ngx_cpymem(b->last, param[i].key.data, param[i].key.len);
                            *b->last++ = '=';
                            *b->last++ = '"';
                            b->last = ngx_cpymem(b->last, param[i].value.data, param[i].value.len);
                            *b->last++ = '"';
                        }
                ...
                ngx_memzero(params, (NGX_HTTP_SSI_MAX_PARAMS + 1) * sizeof(ngx_str_t *));
                ...
                for (i = 0; i < ctx->params.nelts; i++) {
                    ...
                    for (prm = cmd->params; prm->name.len; prm++) {
                        ...
                        if (!prm->multiple) {
                            if (params[prm->index]) {
                                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "duplicate \"%V\" parameter "
                                                                                  "in \"%V\" SSI command", &param[i].key, &ctx->command);
                    ...
                    if (prm->name.len == 0) {
                        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "invalid parameter name: \"%V\" "
                                                                          "in \"%V\" SSI command", &param[i].key, &ctx->command);
                ...
                for (prm = cmd->params; prm->name.len; prm++) {
                    if (prm->mandatory && params[prm->index] == 0) {
                        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "mandatory \"%V\" parameter is absent "
                                                                          "in \"%V\" SSI command", &prm->name, &ctx->command);
        ...
        switch (state) {
            ...
            case ssi_precommand_state:
                switch (ch) {
                    ...
                    default:
                        ctx->command.len = 1;
                        ctx->command.data = ngx_pnalloc(r->pool, NGX_HTTP_SSI_COMMAND_LEN);
            ...
            case ssi_preparam_state:
                switch (ch) {
                    ...
                    default:
                        ...
                        ctx->param->key.len = 1;
                        ctx->param->key.data = ngx_pnalloc(r->pool, NGX_HTTP_SSI_PARAM_LEN);
                        ...
                        if (ctx->value_buf == NULL) {
                            ctx->param->value.data = ngx_pnalloc(r->pool, ctx->value_len + 1);
            ...
            case ssi_param_state:
                switch (ch) {
                    ...
                    case '-':
                        ...
                        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "invalid \"%V\" parameter in \"%V\" SSI command",
                    ...
                    default:
                        if (ctx->param->key.len == NGX_HTTP_SSI_PARAM_LEN) {
                            state = ssi_error_state;
                            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "too long \"%V%c...\" parameter in "
                                                                              "\"%V\" SSI command", &ctx->param->key, ch, &ctx->command);
                            break;
                        }
            ...
            case ssi_preequal_state:
                switch (ch) {
                    ...
                    default:
                        ...
                        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "unexpected \"%c\" symbol after \"%V\" "
                                                                          "parameter in \"%V\" SSI command", ch, &ctx->param->key,
                                      &ctx->command);
                        break;
                }
            ...
            case ssi_prevalue_state:
                switch (ch) {
                    ...
                    default:
                        ...
                        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "unexpected \"%c\" symbol before value of "
                                                                          "\"%V\" parameter in \"%V\" SSI command", ch, &ctx->param->key,
                                      &ctx->command);
                        break;
                }
                ...
                switch (ch) {
                    ...
                    default:
                        if (ctx->param->value.len == ctx->value_len) {
                            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "too long \"%V%c...\" value of \"%V\" "
                                                                              "parameter in \"%V\" SSI command", &ctx->param->value, ch,
                                          &ctx->param->key, &ctx->command);
                            state = ssi_error_state;
                            break;
                        }
            ...
            case ssi_postparam_state:
                ...
                if (ctx->param->value.len + 1 < ctx->value_len / 2) {
                    ...
                    ngx_memcpy(value, ctx->param->value.data, ctx->param->value.len);
                ...
                switch (ch) {
                    ...
                    default:
                        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "unexpected \"%c\" symbol after \"%V\" value "
                                                                          "of \"%V\" parameter in \"%V\" SSI command", ch,
                                      &ctx->param->value, &ctx->param->key, &ctx->command);
                        state = ssi_error_state;
                        break;
                }
            ...
            case ssi_comment_end0_state:
                switch (ch) {
                    ...
                    default:
                        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "unexpected \"%c\" symbol in \"%V\" SSI command", ch,
                                      &ctx->command);
                        state = ssi_error_state;
                        break;
                }
...
static ngx_int_t ngx_http_ssi_evaluate_string(ngx_http_request_t *r, ngx_http_ssi_ctx_t *ctx, ngx_str_t *text, ngx_uint_t flags) {
    ...
    while (i < text->len) {
        ...
        if (text->data[i] == '$') {
            ...
            if (bracket) {
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "the closing bracket in \"%V\" "
...
static ngx_int_t ngx_http_ssi_regex_match(ngx_http_request_t *r, ngx_str_t *pattern, ngx_str_t *str) {
    ...
    if (rc < NGX_REGEX_NO_MATCHED) {
        ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0, ngx_regex_exec_n
                " failed: %d on \"%V\" using \"%V\"", rc, str, pattern);
        return NGX_HTTP_SSI_ERROR;
    }
    ...
    if (rgc.named_captures > 0) {
        ...
        if (ctx->variables == NULL) {
            ctx->variables = ngx_list_create(r->pool, 4, sizeof(ngx_http_ssi_var_t));
            if (ctx->variables == NULL) {
                return NGX_ERROR;
            }
        }
...
static ngx_int_t ngx_http_ssi_include(ngx_http_request_t *r, ngx_http_ssi_ctx_t *ctx, ngx_str_t **params) {
    ...
    if (set && stub) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "\"set\" and \"stub\" cannot be used together "
    ...
    if (stub) {
        if (mctx->blocks) {
            ...
            for (i = 0; i < mctx->blocks->nelts; i++) {
                if (stub->len == bl[i].name.len && ngx_strncmp(stub->data, bl[i].name.data, stub->len) == 0) {
                    goto found;
                }
            }
        }
...
static ngx_int_t ngx_http_ssi_set(ngx_http_request_t *r, ngx_http_ssi_ctx_t *ctx, ngx_str_t **params) {
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "ssi set \"%V\" \"%V\"", name, value);
...
static ngx_int_t ngx_http_ssi_if(ngx_http_request_t *r, ngx_http_ssi_ctx_t *ctx, ngx_str_t **params) {
    ...
    for (p = left.data; p < last; p++) {
        ...
        if ((*p >= 'a' && *p <= 'z') || (*p >= '0' && *p <= '9') || *p == '$' || *p == '{' || *p == '}' || *p == '_' || *p == '"' ||
            *p == '\'') {
            continue;
        }
...
static ngx_int_t ngx_http_ssi_date_gmt_local_variable(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t gmt) {
    ...
    if (timefmt->len == sizeof("%s") - 1 && timefmt->data[0] == '%' && timefmt->data[1] == 's') {
...
static char *ngx_http_ssi_init_main_conf(ngx_conf_t *cf, void *conf) {
    ...
    if (ngx_hash_init(&hash, smcf->commands.keys.elts, smcf->commands.keys.nelts) != NGX_OK) {
