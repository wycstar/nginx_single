...
static ngx_http_v2_push_header_t ngx_http_v2_push_headers[] = {{ngx_string(":authority"),      NGX_HTTP_V2_AUTHORITY_INDEX,       offsetof(
                                                                                                                                          ngx_http_headers_in_t,
                                                                                                                                          host)},

                                                               {ngx_string("accept-encoding"), NGX_HTTP_V2_ACCEPT_ENCODING_INDEX, offsetof(
                                                                                                                                          ngx_http_headers_in_t,
                                                                                                                                          accept_encoding)},

                                                               {ngx_string("accept-language"), NGX_HTTP_V2_ACCEPT_LANGUAGE_INDEX, offsetof(
                                                                                                                                          ngx_http_headers_in_t,
                                                                                                                                          accept_language)},

                                                               {ngx_string("user-agent"),      NGX_HTTP_V2_USER_AGENT_INDEX,      offsetof(
                                                                                                                                          ngx_http_headers_in_t,
                                                                                                                                          user_agent)},};
...
static ngx_http_v2_out_frame_t *ngx_http_v2_create_push_frame(ngx_http_request_t *r, u_char *pos, u_char *end);
static ngx_http_v2_out_frame_t *ngx_http_v2_create_trailers_frame(ngx_http_request_t *r);

static ngx_chain_t *ngx_http_v2_send_chain(ngx_connection_t *fc, ngx_chain_t *in, off_t limit);

static ngx_chain_t *ngx_http_v2_filter_get_shadow(ngx_http_v2_stream_t *stream, ngx_buf_t *buf, off_t offset, off_t size);
static ngx_http_v2_out_frame_t *
ngx_http_v2_filter_get_data_frame(ngx_http_v2_stream_t *stream, size_t len, ngx_chain_t *first, ngx_chain_t *last);

static ngx_inline ngx_int_t ngx_http_v2_flow_control(ngx_http_v2_connection_t *h2c, ngx_http_v2_stream_t *stream);
static void ngx_http_v2_waiting_queue(ngx_http_v2_connection_t *h2c, ngx_http_v2_stream_t *stream);

static ngx_inline ngx_int_t ngx_http_v2_filter_send(ngx_connection_t *fc, ngx_http_v2_stream_t *stream);

static ngx_int_t ngx_http_v2_headers_frame_handler(ngx_http_v2_connection_t *h2c, ngx_http_v2_out_frame_t *frame);
static ngx_int_t ngx_http_v2_push_frame_handler(ngx_http_v2_connection_t *h2c, ngx_http_v2_out_frame_t *frame);
static ngx_int_t ngx_http_v2_data_frame_handler(ngx_http_v2_connection_t *h2c, ngx_http_v2_out_frame_t *frame);
static ngx_inline void ngx_http_v2_handle_frame(ngx_http_v2_stream_t *stream, ngx_http_v2_out_frame_t *frame);
...
static ngx_http_module_t ngx_http_v2_filter_module_ctx = {NULL,                                  /* preconfiguration */
...
ngx_module_t ngx_http_v2_filter_module = {NGX_MODULE_V1, &ngx_http_v2_filter_module_ctx,        /* module context */
...
static ngx_int_t ngx_http_v2_header_filter(ngx_http_request_t *r) {
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http2 header filter");
    ...
    if (!h2c->push_disabled && !h2c->goaway && stream->node->id % 2 == 1 && r->method != NGX_HTTP_HEAD) {
    ...
    if (h2c->table_update) {
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, fc->log, 0, "http2 table size update: 0");
        *pos++ = (1 << 5) | 0;
        h2c->table_update = 0;
    }

    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, fc->log, 0, "http2 output header: \":status: %03ui\"", r->headers_out.status);
    ...
    if (r->headers_out.server == NULL) {
        ...
        if (clcf->server_tokens == NGX_HTTP_SERVER_TOKENS_ON) {
            ngx_log_debug1(NGX_LOG_DEBUG_HTTP, fc->log, 0, "http2 output header: \"server: %s\"", NGINX_VER);

        } else if (clcf->server_tokens == NGX_HTTP_SERVER_TOKENS_BUILD) {
            ngx_log_debug1(NGX_LOG_DEBUG_HTTP, fc->log, 0, "http2 output header: \"server: %s\"", NGINX_VER_BUILD);

        } else {
            ngx_log_debug0(NGX_LOG_DEBUG_HTTP, fc->log, 0, "http2 output header: \"server: nginx\"");
        }
    ...
    if (r->headers_out.date == NULL) {
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, fc->log, 0, "http2 output header: \"date: %V\"", &ngx_cached_http_time);

        *pos++ = ngx_http_v2_inc_indexed(NGX_HTTP_V2_DATE_INDEX);
        pos = ngx_http_v2_write_value(pos, ngx_cached_http_time.data, ngx_cached_http_time.len, tmp);
    }
    ...
    if (r->headers_out.content_type.len) {
        *pos++ = ngx_http_v2_inc_indexed(NGX_HTTP_V2_CONTENT_TYPE_INDEX);
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, fc->log, 0, "http2 output header: \"content-type: %V\"", &r->headers_out.content_type);

        pos = ngx_http_v2_write_value(pos, r->headers_out.content_type.data, r->headers_out.content_type.len, tmp);
    }

    if (r->headers_out.content_length == NULL && r->headers_out.content_length_n >= 0) {
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, fc->log, 0, "http2 output header: \"content-length: %O\"", r->headers_out.content_length_n);
    ...
    if (r->headers_out.last_modified == NULL && r->headers_out.last_modified_time != -1) {
        *pos++ = ngx_http_v2_inc_indexed(NGX_HTTP_V2_LAST_MODIFIED_INDEX);
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, fc->log, 0, "http2 output header: \"last-modified: %*s\"", len, pos);
    ...
    if (r->headers_out.location && r->headers_out.location->value.len) {
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, fc->log, 0, "http2 output header: \"location: %V\"", &r->headers_out.location->value);

        *pos++ = ngx_http_v2_inc_indexed(NGX_HTTP_V2_LOCATION_INDEX);
        pos = ngx_http_v2_write_value(pos, r->headers_out.location->value.data, r->headers_out.location->value.len, tmp);
    }

#if (NGX_HTTP_GZIP)
    if (r->gzip_vary) {
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, fc->log, 0, "http2 output header: \"vary: Accept-Encoding\"");
    ...
    for (i = 0; /* void */; i++) {
        ...
        *pos++ = 0;
        ...
        pos = ngx_http_v2_write_name(pos, header[i].key.data, header[i].key.len, tmp);

        pos = ngx_http_v2_write_value(pos, header[i].value.data, header[i].value.len, tmp);
    }

    fin = r->header_only || (r->headers_out.content_length_n == 0 && !r->expect_trailers);
...
static ngx_int_t ngx_http_v2_push_resources(ngx_http_request_t *r) {
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http2 push resources");
    ...
    for (i = 0; i < r->headers_out.link.nelts; i++) {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http2 parse link: \"%V\"", &h[i]->value);
...
static ngx_int_t ngx_http_v2_push_resource(ngx_http_request_t *r, ngx_str_t *path, ngx_str_t *binary) {
    ...
    if (!ngx_path_separator(path->data[0])) {
        ngx_log_error(NGX_LOG_WARN, fc->log, 0, "non-absolute path \"%V\" not pushed", path);
        return NGX_DECLINED;
    }

    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0, "http2 pushing:%ui limit:%ui", h2c->pushing, h2c->concurrent_pushes);
    ...
    len = (h2c->table_update ? 1 : 0) + 1 + 1 + NGX_HTTP_V2_INT_OCTETS + path->len + 1 + NGX_HTTP_V2_INT_OCTETS + r->schema.len;
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, fc->log, 0, "http2 push header: \":method: GET\"");

    *pos++ = ngx_http_v2_indexed(NGX_HTTP_V2_METHOD_GET_INDEX);

    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, fc->log, 0, "http2 push header: \":path: %V\"", path);

    *pos++ = ngx_http_v2_inc_indexed(NGX_HTTP_V2_PATH_INDEX);
    pos = ngx_http_v2_write_value(pos, path->data, path->len, tmp);

    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, fc->log, 0, "http2 push header: \":scheme: %V\"", &r->schema);

    if (r->schema.len == 5 && ngx_strncmp(r->schema.data, "https", 5) == 0) {
        *pos++ = ngx_http_v2_indexed(NGX_HTTP_V2_SCHEME_HTTPS_INDEX);

    } else if (r->schema.len == 4 && ngx_strncmp(r->schema.data, "http", 4) == 0) {
    ...
    for (i = 0; i < NGX_HTTP_V2_PUSH_HEADERS; i++) {
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, fc->log, 0, "http2 push header: \"%V: %V\"", &ph[i].name, &(*h)->value);
...
static ngx_http_v2_out_frame_t *ngx_http_v2_create_headers_frame(ngx_http_request_t *r, u_char *pos, u_char *end, ngx_uint_t fin) {
    ...
    for (;;) {
        ...
        *ll = cl;
        ...
        ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http2:%ui create HEADERS frame %p: len:%uz fin:%ui", stream->node->id,
                       frame, frame->length, fin);
...
static ngx_http_v2_out_frame_t *ngx_http_v2_create_push_frame(ngx_http_request_t *r, u_char *pos, u_char *end) {
    ...
    for (;;) {
        ...
        b = ngx_create_temp_buf(r->pool, NGX_HTTP_V2_FRAME_HEADER_SIZE +
                                         ((type == NGX_HTTP_V2_PUSH_PROMISE_FRAME) ? NGX_HTTP_V2_STREAM_ID_SIZE : 0));
        if (b == NULL) {
            return NULL;
        }
        ...
        *ll = cl;
        ...
        ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http2:%ui create PUSH_PROMISE frame %p: "
                                                                  "sid:%ui len:%uz", stream->node->id, frame, h2c->last_push,
                       frame->length);
...
static ngx_http_v2_out_frame_t *ngx_http_v2_create_trailers_frame(ngx_http_request_t *r) {
    ...
    for (i = 0; /* void */; i++) {
        ...
        if (header[i].key.len > NGX_HTTP_V2_MAX_FIELD) {
            ngx_log_error(NGX_LOG_CRIT, fc->log, 0, "too long response trailer name: \"%V\"", &header[i].key);
            return NULL;
        }

        if (header[i].value.len > NGX_HTTP_V2_MAX_FIELD) {
            ngx_log_error(NGX_LOG_CRIT, fc->log, 0, "too long response trailer value: \"%V: %V\"", &header[i].key, &header[i].value);
            return NULL;
        }
...
static ngx_chain_t *ngx_http_v2_send_chain(ngx_connection_t *fc, ngx_chain_t *in, off_t limit) {
    ...
    if (in->buf->tag == (ngx_buf_tag_t) &ngx_http_v2_filter_get_shadow) {
        ...
        offset = ngx_buf_in_memory(in->buf) ? (cl->buf->pos - in->buf->pos) : (cl->buf->file_pos - in->buf->file_pos);
    ...
    frame_size = (h2lcf->chunk_size < h2c->frame_size) ? h2lcf->chunk_size : h2c->frame_size;
...
ngx_http_v2_filter_get_data_frame(ngx_http_v2_stream_t *stream, size_t len, ngx_chain_t *first, ngx_chain_t *last) {
    ...
    if (frame) {
        ...
        frame = ngx_palloc(stream->request->pool, sizeof(ngx_http_v2_out_frame_t));
        if (frame == NULL) {
            return NULL;
        }
    }
    ...
    ngx_log_debug4(NGX_LOG_DEBUG_HTTP, stream->request->connection->log, 0, "http2:%ui create DATA frame %p: len:%uz flags:%ui",
                   stream->node->id, frame, len, (ngx_uint_t) flags);

    cl = ngx_chain_get_free_buf(stream->request->pool, &stream->free_frame_headers);
    if (cl == NULL) {
        return NULL;
    }
    ...
    buf->last = ngx_http_v2_write_len_and_type(buf->last, len, NGX_HTTP_V2_DATA_FRAME);
    *buf->last++ = flags;
...
static ngx_inline ngx_int_t ngx_http_v2_flow_control(ngx_http_v2_connection_t *h2c, ngx_http_v2_stream_t *stream) {
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0, "http2:%ui windows: conn:%uz stream:%z", stream->node->id, h2c->send_window,
                   stream->send_window);
...
static void ngx_http_v2_waiting_queue(ngx_http_v2_connection_t *h2c, ngx_http_v2_stream_t *stream) {
    ...
    for (q = ngx_queue_last(&h2c->waiting); q != ngx_queue_sentinel(&h2c->waiting); q = ngx_queue_prev(q)) {
        s = ngx_queue_data(q, ngx_http_v2_stream_t, queue);

        if (s->node->rank < stream->node->rank ||
            (s->node->rank == stream->node->rank && s->node->rel_weight >= stream->node->rel_weight)) {
            break;
        }
    }
...
static ngx_int_t ngx_http_v2_headers_frame_handler(ngx_http_v2_connection_t *h2c, ngx_http_v2_out_frame_t *frame) {
    ...
    for (;;) {
        if (cl->buf->pos != cl->buf->last) {
            ...
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0, "http2:%ui HEADERS frame %p was sent partially", stream->node->id,
                           frame);
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0, "http2:%ui HEADERS frame %p was sent", stream->node->id, frame);

    stream->request->header_size += NGX_HTTP_V2_FRAME_HEADER_SIZE + frame->length;
...
static ngx_int_t ngx_http_v2_push_frame_handler(ngx_http_v2_connection_t *h2c, ngx_http_v2_out_frame_t *frame) {
    ...
    for (;;) {
        if (cl->buf->pos != cl->buf->last) {
            ...
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0, "http2:%ui PUSH_PROMISE frame %p was sent partially",
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0, "http2:%ui PUSH_PROMISE frame %p was sent", stream->node->id, frame);
...
static ngx_int_t ngx_http_v2_data_frame_handler(ngx_http_v2_connection_t *h2c, ngx_http_v2_out_frame_t *frame) {
    ...
    if (cl->buf->tag == (ngx_buf_tag_t) &ngx_http_v2_module) {
        ...
        if (cl->buf->pos != cl->buf->last) {
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0, "http2:%ui DATA frame %p was sent partially", stream->node->id,
                           frame);
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0, "http2:%ui DATA frame %p was sent", stream->node->id, frame);
