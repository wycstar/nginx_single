...
static ngx_command_t ngx_mail_commands[] = {
        ...
        {ngx_string("mail"), NGX_MAIN_CONF | NGX_CONF_BLOCK | NGX_CONF_NOARGS, ngx_mail_block, 0, 0, NULL},
...
ngx_module_t ngx_mail_module = {NGX_MODULE_V1, &ngx_mail_module_ctx,                  /* module context */
...
static char *ngx_mail_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    *cf = pcf;
    ...
    if (ngx_array_init(&ports, cf->temp_pool, 4, sizeof(ngx_mail_conf_port_t)) != NGX_OK) {
        return NGX_CONF_ERROR;
    }
...
static ngx_int_t ngx_mail_add_ports(ngx_conf_t *cf, ngx_array_t *ports, ngx_mail_listen_t *listen) {
    ...
    if (ngx_array_init(&port->addrs, cf->temp_pool, 2, sizeof(ngx_mail_conf_addr_t)) != NGX_OK) {
        return NGX_ERROR;
    }
...
static char *ngx_mail_optimize_servers(ngx_conf_t *cf, ngx_array_t *ports) {
    ...
    for (p = 0; p < ports->nelts; p++) {
        ...
        ngx_sort(port[p].addrs.elts, (size_t) port[p].addrs.nelts, sizeof(ngx_mail_conf_addr_t), ngx_mail_cmp_conf_addrs);
