...
void ngx_event_acceptex(ngx_event_t *rev) {
    ...
    if (rev->ovlp.error) {
        ngx_log_error(NGX_LOG_CRIT, c->log, rev->ovlp.error, "AcceptEx() %V failed", &ls->addr_text);
        return;
    }
    ...
    ngx_getacceptexsockaddrs(c->buffer->pos, ls->post_accept_buffer_size, ls->socklen + 16, ls->socklen + 16, &c->local_sockaddr,
                             &c->local_socklen, &c->sockaddr, &c->socklen);
...
ngx_int_t ngx_event_post_acceptex(ngx_listening_t *ls, ngx_uint_t n) {
    ...
    for (i = 0; i < n; i++) {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_EVENT, &ls->log, 0, ngx_socket_n
                " s:%d", s);

        if (s == (ngx_socket_t) -1) {
            ngx_log_error(NGX_LOG_ALERT, &ls->log, ngx_socket_errno, ngx_socket_n
                    " failed");
        ...
        c->buffer = ngx_create_temp_buf(c->pool, ls->post_accept_buffer_size + 2 * (ls->socklen + 16));
        if (c->buffer == NULL) {
            ngx_close_posted_connection(c);
            return NGX_ERROR;
        }
        ...
        *log = ls->log;
        ...
        if (ngx_acceptex(ls->fd, s, c->buffer->pos, ls->post_accept_buffer_size, ls->socklen + 16, ls->socklen + 16, &rcvd,
                         (LPOVERLAPPED) & rev->ovlp) == 0) {
...
static void ngx_close_posted_connection(ngx_connection_t *c) {
    ...
    if (ngx_close_socket(fd) == -1) {
        ngx_log_error(NGX_LOG_ALERT, c->log, ngx_socket_errno, ngx_close_socket_n
                " failed");
    }
