...
static ngx_int_t ngx_eventport_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags);
static ngx_int_t ngx_eventport_del_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags);
static ngx_int_t ngx_eventport_notify(ngx_event_handler_pt handler);
static ngx_int_t ngx_eventport_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags);
...
static ngx_command_t ngx_eventport_commands[] = {
        ...
        {ngx_string("eventport_events"), NGX_EVENT_CONF | NGX_CONF_TAKE1, ngx_conf_set_num_slot, 0, offsetof(ngx_eventport_conf_t, events),
         NULL},

        ngx_null_command};


static ngx_event_module_t ngx_eventport_module_ctx = {&eventport_name, ngx_eventport_create_conf,             /* create configuration */
...
ngx_module_t ngx_eventport_module = {NGX_MODULE_V1, &ngx_eventport_module_ctx,             /* module context */
...
static ngx_int_t ngx_eventport_init(ngx_cycle_t *cycle, ngx_msec_t timer) {
    ...
    if (ep == -1) {
        ...
        if (ep == -1) {
            ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno, "port_create() failed");
            return NGX_ERROR;
        }
    ...
    if (nevents < epcf->events) {
        ...
        event_list = ngx_alloc(sizeof(port_event_t) * epcf->events, cycle->log);
        if (event_list == NULL) {
            return NGX_ERROR;
        }
    }
    ...
    if (timer) {
        ...
        if (timer_create(CLOCK_REALTIME, &sev, &event_timer) == -1) {
            ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno, "timer_create() failed");
            return NGX_ERROR;
        }
        ...
        if (timer_settime(event_timer, 0, &its, NULL) == -1) {
            ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno, "timer_settime() failed");
            return NGX_ERROR;
        }
...
static void ngx_eventport_done(ngx_cycle_t *cycle) {
    if (event_timer != (timer_t) -1) {
        if (timer_delete(event_timer) == -1) {
            ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_errno, "timer_delete() failed");
        }
    ...
    if (close(ep) == -1) {
        ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_errno, "close() event port failed");
    }
...
static ngx_int_t ngx_eventport_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags) {
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_EVENT, ev->log, 0, "eventport add event: fd:%d ev:%04Xi", c->fd, events);

    if (port_associate(ep, PORT_SOURCE_FD, c->fd, events, (void *) ((uintptr_t) ev | ev->instance)) == -1) {
        ngx_log_error(NGX_LOG_ALERT, ev->log, ngx_errno, "port_associate() failed");
        return NGX_ERROR;
    }
...
static ngx_int_t ngx_eventport_del_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags) {
    ...
    if (e->oneshot) {
        ngx_log_debug2(NGX_LOG_DEBUG_EVENT, ev->log, 0, "eventport change event: fd:%d ev:%04Xi", c->fd, event);

        if (port_associate(ep, PORT_SOURCE_FD, c->fd, event, (void *) ((uintptr_t) ev | ev->instance)) == -1) {
            ngx_log_error(NGX_LOG_ALERT, ev->log, ngx_errno, "port_associate() failed");
            return NGX_ERROR;
        }

    } else {
        ngx_log_debug1(NGX_LOG_DEBUG_EVENT, ev->log, 0, "eventport del event: fd:%d", c->fd);

        if (port_dissociate(ep, PORT_SOURCE_FD, c->fd) == -1) {
            ngx_log_error(NGX_LOG_ALERT, ev->log, ngx_errno, "port_dissociate() failed");
            return NGX_ERROR;
        }
    }
...
static ngx_int_t ngx_eventport_notify(ngx_event_handler_pt handler) {
    ...
    if (port_send(ep, 0, &notify_event) != 0) {
        ngx_log_error(NGX_LOG_ALERT, notify_event.log, ngx_errno, "port_send() failed");
        return NGX_ERROR;
    }
...
static ngx_int_t ngx_eventport_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "eventport timer: %M", timer);
    ...
    if (n == -1) {
        if (err == ETIME) {
            ...
            ngx_log_error(NGX_LOG_ALERT, cycle->log, 0, "port_getn() returned no events without timeout");
            return NGX_ERROR;
        }
        ...
        switch (event_list[i].portev_source) {
            ...
            case PORT_SOURCE_FD:
                ...
                if (ev->closed || ev->instance != instance) {
                    ...
                    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "eventport: stale event %p", ev);
                    continue;
                }

                revents = event_list[i].portev_events;

                ngx_log_debug2(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "eventport: fd:%d, ev:%04Xd", (int) event_list[i].portev_object,
                               revents);

                if (revents & (POLLERR | POLLHUP | POLLNVAL)) {
                    ngx_log_debug2(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "port_getn() error fd:%d ev:%04Xd",
                                   (int) event_list[i].portev_object, revents);
                }

                if (revents & ~(POLLIN | POLLOUT | POLLERR | POLLHUP | POLLNVAL)) {
                    ngx_log_error(NGX_LOG_ALERT, cycle->log, 0, "strange port_getn() events fd:%d ev:%04Xd",
                                  (int) event_list[i].portev_object, revents);
                }
                ...
                if (revents & POLLIN) {
                    ...
                    if (rev->accept) {
                        ...
                        if (port_associate(ep, PORT_SOURCE_FD, c->fd, POLLIN, (void *) ((uintptr_t) ev | ev->instance)) == -1) {
                            ngx_log_error(NGX_LOG_ALERT, ev->log, ngx_errno, "port_associate() failed");
                            return NGX_ERROR;
                        }
                    }
            ...
            default:
                ngx_log_error(NGX_LOG_ALERT, cycle->log, 0, "unexpected eventport object %d", (int) event_list[i].portev_object);
                continue;
        }
    }
