...
static ngx_event_module_t ngx_poll_module_ctx = {&poll_name, NULL,                                  /* create configuration */
...
ngx_module_t ngx_poll_module = {NGX_MODULE_V1, &ngx_poll_module_ctx,                  /* module context */
...
static ngx_int_t ngx_poll_init(ngx_cycle_t *cycle, ngx_msec_t timer) {
    ...
    if (ngx_process >= NGX_PROCESS_WORKER || cycle->old_cycle == NULL || cycle->old_cycle->connection_n < cycle->connection_n) {
        ...
        index = ngx_alloc(sizeof(ngx_connection_t *) * cycle->connection_n, cycle->log);
        if (index == NULL) {
            return NGX_ERROR;
        }

        if (event_index) {
            ngx_memcpy(index, event_index, sizeof(ngx_connection_t *) * nevents);
...
static ngx_int_t ngx_poll_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags) {
    ...
    if (ev->index != NGX_INVALID_INDEX) {
        ngx_log_error(NGX_LOG_ALERT, ev->log, 0, "poll event fd:%d ev:%i is already set", c->fd, event);
        return NGX_OK;
    }
...
static ngx_int_t ngx_poll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags) {
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "poll ready %d of %ui", ready, nevents);
    ...
    if (ready == 0) {
        ...
        ngx_log_error(NGX_LOG_ALERT, cycle->log, 0, "WSAPoll() returned no events without timeout");
        return NGX_ERROR;
    }
    ...
    for (i = 0; i < nevents && ready; i++) {
...
#if 1
        ngx_log_debug4(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "poll: %ui: fd:%d ev:%04Xd rev:%04Xd", i, event_list[i].fd, event_list[i].events,
                       revents);
#else
        if (revents) {
        ...
        if (revents & POLLNVAL) {
            ngx_log_error(NGX_LOG_ALERT, cycle->log, 0, "poll() error fd:%d ev:%04Xd rev:%04Xd", event_list[i].fd, event_list[i].events,
                          revents);
        }

        if (revents & ~(POLLIN | POLLOUT | POLLERR | POLLHUP | POLLNVAL)) {
            ngx_log_error(NGX_LOG_ALERT, cycle->log, 0, "strange poll() events fd:%d ev:%04Xd rev:%04Xd", event_list[i].fd,
                          event_list[i].events, revents);
        }
