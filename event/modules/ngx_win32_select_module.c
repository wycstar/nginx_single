...
static ngx_event_module_t ngx_select_module_ctx = {&select_name, NULL,                                  /* create configuration */
...
ngx_module_t ngx_select_module = {NGX_MODULE_V1, &ngx_select_module_ctx,                /* module context */
...
static ngx_int_t ngx_select_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags) {
    ...
    if ((event == NGX_READ_EVENT && max_read >= FD_SETSIZE) || (event == NGX_WRITE_EVENT && max_write >= FD_SETSIZE)) {
        ngx_log_error(NGX_LOG_ERR, ev->log, 0, "maximum number of descriptors "
...
static ngx_int_t ngx_select_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags) {
    ...
    if (max_read || max_write) {
        ready = select(0, &work_read_fd_set, &work_write_fd_set, &work_except_fd_set, tp);
    ...
    for (i = 0; i < nevents; i++) {
        ...
        if (ev->write) {
            ...
            if (FD_ISSET(c->fd, &work_except_fd_set)) {
                found++;
                ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "select except %d", c->fd);
            }
