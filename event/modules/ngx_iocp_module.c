...
static ngx_command_t ngx_iocp_commands[] = {
        ...
        {ngx_string("iocp_threads"), NGX_EVENT_CONF | NGX_CONF_TAKE1, ngx_conf_set_num_slot, 0, offsetof(ngx_iocp_conf_t, threads), NULL},

        {ngx_string("post_acceptex"), NGX_EVENT_CONF | NGX_CONF_TAKE1, ngx_conf_set_num_slot, 0, offsetof(ngx_iocp_conf_t, post_acceptex),
         NULL},

        {ngx_string("acceptex_read"), NGX_EVENT_CONF | NGX_CONF_FLAG, ngx_conf_set_flag_slot, 0, offsetof(ngx_iocp_conf_t, acceptex_read),
         NULL},

        ngx_null_command};


static ngx_event_module_t ngx_iocp_module_ctx = {&iocp_name, ngx_iocp_create_conf,                  /* create configuration */
...
ngx_module_t ngx_iocp_module = {NGX_MODULE_V1, &ngx_iocp_module_ctx,                  /* module context */
...
ngx_os_io_t ngx_iocp_io = {ngx_overlapped_wsarecv, NULL, ngx_udp_overlapped_wsarecv, NULL, NULL, NULL, ngx_overlapped_wsasend_chain, 0};
...
static ngx_int_t ngx_iocp_init(ngx_cycle_t *cycle, ngx_msec_t timer) {
    ...
    if (iocp == NULL) {
        ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_errno, "CreateIoCompletionPort() failed");
        return NGX_ERROR;
    }
    ...
    if (timer_thread == NULL) {
        ...
        if (ngx_create_thread(&timer_thread, ngx_iocp_timer, &msec, cycle->log) != 0) {
            return NGX_ERROR;
        }
    }
...
static void ngx_iocp_done(ngx_cycle_t *cycle) {
    if (CloseHandle(iocp) == -1) {
        ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_errno, "iocp CloseHandle() failed");
    }
...
static ngx_int_t ngx_iocp_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags) {
    ...
    ngx_log_debug4(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "iocp: %d b:%d k:%d ov:%p", rc, bytes, key, ovlp);

    if (timer != INFINITE) {
        delta = ngx_current_msec - delta;

        ngx_log_debug2(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "iocp timer: %M, delta: %M", timer, delta);
    }

    if (err) {
        if (ovlp == NULL) {
            if (err != WAIT_TIMEOUT) {
                ngx_log_error(NGX_LOG_ALERT, cycle->log, err, "GetQueuedCompletionStatus() failed");
    ...
    if (ovlp == NULL) {
        ngx_log_error(NGX_LOG_ALERT, cycle->log, 0, "GetQueuedCompletionStatus() returned no operation");
        return NGX_ERROR;
    }
    ...
    if (err == ERROR_NETNAME_DELETED /* the socket was closed */
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, err, "iocp: aborted event %p", ev);

        return NGX_OK;
    }

    if (err) {
        ngx_log_error(NGX_LOG_ALERT, cycle->log, err, "GetQueuedCompletionStatus() returned operation error");
    }
