...
int ngx_iocp_wait_connect(ngx_connection_t *c) {
    for (;;) {
        ...
        if (nconnects < NGX_MAX_PENDING_CONN) {
            ...
            if (SetEvent(pending_connect_event) == 0) {
                ngx_log_error(NGX_LOG_ALERT, c->log, ngx_errno, "SetEvent() failed");
                return NGX_ERROR;
            ...
            LeaveCriticalSection(&connect_lock);
            ngx_log_error(NGX_LOG_NOTICE, c->log, 0, "max number of pending connect()s is %d", NGX_MAX_PENDING_CONN);
            msleep(100);
        }
    ...
    int ngx_iocp_new_thread(int main) {
        ...
        if (main) {
            ...
            if (pending_connect_event == INVALID_HANDLE_VALUE) {
                ngx_log_error(NGX_LOG_ALERT, c->log, ngx_errno, "CreateThread() failed");
                return NGX_ERROR;
            }
        }

        if (CreateThread(NULL, 0, ngx_iocp_wait_events, main, 0, &id) == INVALID_HANDLE_VALUE) {
            ngx_log_error(NGX_LOG_ALERT, c->log, ngx_errno, "CreateThread() failed");
            return NGX_ERROR;
        }
    ...
    int ngx_iocp_new_connect() {
        ...
        if (events[nevents] == INVALID_HANDLE_VALUE) {
            ngx_log_error(NGX_LOG_ALERT, c->log, ngx_socket_errno, "WSACreateEvent() failed");
            return NGX_ERROR;
        }

        if (WSAEventSelect(c->fd, events[nevents], FD_CONNECT) == -1)
            ngx_log_error(NGX_LOG_ALERT, c->log, ngx_socket_errno, "WSAEventSelect() failed");
        return NGX_ERROR;
    }
...
void ngx_iocp_wait_events(int main) {
    ...
    for (;;) {
        ...
        n = WSAWaitForMultipleEvents(nevents - offset, events[offset], 0, timeout, 0);
        if (n == WAIT_FAILED) {
            ngx_log_error(NGX_LOG_ALERT, log, ngx_socket_errno, "WSAWaitForMultipleEvents() failed");
            continue;
        }
        ...
        if (n == WAIT_TIMEOUT) {
            ...
            ngx_log_error(NGX_LOG_ALERT, log, 0, "WSAWaitForMultipleEvents() "
        ...
        if (WSAEnumNetworkEvents(c[n].fd, events[n], &ne) == -1) {
            ngx_log_error(NGX_LOG_ALERT, log, ngx_socket_errno, "WSAEnumNetworkEvents() failed");
            continue;
        }
        ...
        if (ne.lNetworkEvents & FD_CONNECT) {
            ...
            if (PostQueuedCompletionStatus(iocp, 0, NGX_IOCP_CONNECT, &conn[n].write->ovlp) == 0) {
                ngx_log_error(NGX_LOG_ALERT, log, ngx_socket_errno, "PostQueuedCompletionStatus() failed");
                continue;
            }
        ...
        ngx_log_error(NGX_LOG_ALERT, c[n].log, 0, "WSAWaitForMultipleEvents() "
